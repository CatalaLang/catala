...
\section{Compiling defaults away}
...
\inferrule[C-Default]{
  ...
  \synjust\synvar{e_\mathrm{cons}}\synrangle\typcolon\synvar{\tau}\\
  ...
  \synkeyword{let}\;\synvar{r_\mathrm{exceptions}}\synequal
  \texttt{fold\_left}\;\synlparen\synlambda\synlparen\synvar{a}\syntyped
  \synvar{\tau}\;\synkeyword{option}\synrparen\;
  \synlparen\synvar{e'}\syntyped
  \synvar{\tau}\;\synkeyword{option}\synrparen\syndot\\
  \synkeyword{match}\;\synlparen\synvar{a}\syncomma\;\synvar{e'}\synrparen\;\synkeyword{with}\;
  \synlparen\synkeyword{None}\syncomma\;\synvar{e'}\synrparen\synarrow\synvar{e'}
  \synmid\synlparen\synkeyword{Some}\;\synvar{a}\syncomma\;\synkeyword{None}\synrparen\synarrow
  \synkeyword{Some}\;\synvar{a}
  \synmid\synlparen\synkeyword{Some}\;\synvar{a}\syncomma\;\synkeyword{Some}\;\synvar{e'}\synrparen\synarrow
  \synerror
  \synrparen\\\synkeyword{None}\;\synlsquare\synvar{e_1'}
  \synlistellipsis\synvar{e_n'}\synrsquare\;\synkeyword{in}\;\synkeyword{match}\;
  \synvar{r_\mathrm{exceptions}}\;\synkeyword{with}\;\synkeyword{Some}\;\synvar{e'}\synarrow 
  \synkeyword{Some}\;\synvar{e'}\synmid\synkeyword{None}\synarrow\\
  \synif \synvar{e_\mathrm{just}'}\synthen \synkeyword{Some}\;\synvar{e_\mathrm{cons}'}\synelse 
  \synkeyword{None}
}

\inferrule[C-EmptyError]{}{
  \synemptydefault\compiles\synkeyword{None}
}

\inferrule[C-Base]{
  \synvar{e}\neq\synemptydefault\\
  \synvar{e}\text{ not a default}
}{
  \synvar{e}\compiles\synkeyword{Some}\;\synvar{e}
}
\end{mathpar}

This supposes 
that default terms are wrapped as \verb|crash_if_not_empty| arguments.
Indeed, since we will be transforming $\tau$ to $\tau\;\text{option}$ inside 
defaults, so one needs to eliminate the \verb|None| case before using the 
value coming from a default evaluation. 
