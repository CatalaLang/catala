...
let rec process_base_typ
  ...
  | Surface.Ast.Data (Surface.Ast.Primitive prim) -> (
    ...
    | Surface.Ast.Var (Some (id, pos)) ->
      ...
      | Some v -> TVar v, pos
      | None -> Message.error ~pos "Specifying type `anything` is not allowed at this point"
  )
...
let process_type (ctxt : context) ((naked_typ, typ_pos) : Surface.Ast.typ) : typ
  ...
  | Surface.Ast.Base base_typ -> process_base_typ ~vars:String.Map.empty ctxt (base_typ, typ_pos)
  | Surface.Ast.Func { arg_typ; return_typ } ->
    let rec get_vars = function
      | Surface.Ast.Primitive (Var (Some (id, pos))), _ -> String.Map.singleton id pos
    ...
    Type.quantify (Type.Var.Set.of_list (String.Map.values vars)) (TArrow (targs, process_base_typ ~vars ctxt return_typ), typ_pos)
