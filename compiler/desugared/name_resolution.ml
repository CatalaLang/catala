...
let attribute_parsers
  : (string * string list * attribute_context list *
...
let handle_extra_attributes context plugin path value pos =
  ...
  | [] ->
    Message.warning ~pos "No plugin registered to handle attribute @{<magenta>#[%s.%s]@}" plugin (String.concat "." path);
    None
  | handlers ->
    match List.find_all (fun (_, p, _, _) -> path = p) handlers with
    | [] ->
      Message.warning ~pos
        ~suggestion:(List.map (fun (_, p, _, _) -> String.concat "." p) handlers)
        "Invalid sub-attribute \"%s\" of plugin %s"
        (String.concat "." path) plugin;
      None
    | handlers ->
      match List.find_opt (fun (_, _, ctx, _) -> List.mem context ctx) handlers with
      | None ->
        Message.warning ~pos
  ...
  | Src ((p1 :: ps, ppos), v, pos) -> (
      ...
      | "debug" -> (
          ...
          | ["print"] ->
            if context <> Expression then
              (Message.warning ~pos
                "Attribute @{<magenta>#[debug.print]@} is not allowed in this context";
               None)
            else
              (match v with
      ...
      | "test" -> (
          ...
          | [] ->
            if v <> Unit then
              (Message.warning ~pos
                "The @{<magenta>#[test]@} attribute doesn't allow specifying a value";
               None)
      ...
      | "doc" -> (
          ...
          | [] ->
            if context = Expression then
             ...
             | _ ->
               Message.warning ~pos
                 "Invalid value for the @{<magenta>#[doc]@} attribute: expecting a string";
               None)
          | ps ->
            Message.warning ~pos:ppos
              "Unknown doc sub-attribute \"%s\""
              (String.concat "." ps);
            None)
      | plugin ->
        if ps = [] then
          ...
          handle_extra_attributes context plugin ps v ppos
    )
  | attr -> Some attr
