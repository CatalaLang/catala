...
type conc_error_content = {
  pos : Pos.t;
  message : string;
  constraints : path_constraint list;
}

type conc_error = conc_error_content
type 'c conc_result = ('c conc_expr, conc_error) result
...
(* FIXME check all uses, the constraints they add, etc. *)
let propagate_result
    (e : 'c conc_result)
    (new_constraints : path_constraint list)
    (f : 'c conc_expr -> 'c conc_result) : 'c conc_result =
  let error_add_constraints content : 'c conc_result =
    Message.emit_debug "propagate error %i" (List.length new_constraints);
    Error { content with constraints = content.constraints @ new_constraints }
  in
  Result.fold ~ok:f ~error:error_add_constraints e

let propagate_result_list l new_constraints f =
  (* FIXME fix order *)
  let rec aux acc constraints = function
    | [] -> f (List.rev acc)
    | e :: r ->
      propagate_result e constraints (fun e ->
          aux (e :: acc) (get_constraints e @ constraints) r)
  in
  aux [] new_constraints l

let propagate_empty_error_result
    (e : 'c conc_expr)
    (f : 'c conc_expr -> 'c conc_result) : 'c conc_result =
  match e with (EEmptyError, _) as e -> Ok e | e -> f e
...
let handle_eq evaluate_operator pos lang e1 e2 =
  ...
  | EStruct { fields = es1; name = s1 }, EStruct { fields = es2; name = s2 } ->
    ...
    && StructField.Map.equal
         (fun e1 e2 ->
           let eq = Result.get_ok (evaluate_operator Eq pos lang [e1; e2]) in
           (* TODO check assumption: cbv so no error? *)
           match Mark.remove eq with ELit (LBool b) -> b | _ -> assert false
           (* should not happen *))
         es1 es2
...
let rec evaluate_expr :
    context -> Cli.backend_lang -> yes conc_expr -> yes conc_result =
 fun ctx lang e ->
  ...
  | EApp { f = e1; args; _ } -> (
    ...
    | EAbs { binder; _ } ->
      ...
      if Bindlib.mbinder_arity binder = List.length args then (
        ...
        (* TODO [Expr.subst]? *)
        propagate_result result (args_constraints @ f_constraints)
        @@ fun result ->
        ...
        Ok (add_conc_info_e r_symb ~constraints result)
  ...
  | EAppOp { op; args; _ } ->
    ...
    propagate_result_list args []
    @@ fun args ->
    let args_constraints = gather_constraints args in
    let result =
      evaluate_operator (evaluate_expr ctx lang) ctx op m lang args
    in
    propagate_result result args_constraints
    @@ fun result ->
    let r_symb = get_symb_expr result in
    let r_constraints = get_constraints result in
  ...
  | EAbs { binder; tys } ->
    ...
    Ok (Expr.unbox (Expr.eabs (Bindlib.box binder) tys m))
  ...
  | ELit l as e ->
    ...
    Ok (add_conc_info_m m (Some symb_expr) ~constraints:[] e)
  ...
  | EStruct { fields = es; name } ->
    ...
    (* FIXME why doesn't this work with EApp? *)
  | EStructAccess { e; name = s; field } -> (
    Message.emit_debug "... it's an EStructAccess";
    propagate_result (evaluate_expr ctx lang e) []
    @@ fun e ->
    propagate_empty_error_result e
    @@ fun e ->
    match Mark.remove e with
    | EStruct { fields = es; name } ->
  ...
  | EInj { name; e; cons } ->
    ...
    Ok (add_conc_info_m m (Some symb_expr) ~constraints concrete)
  | EMatch { e; cases; name } -> (
    ...
    | EInj { e = e1; cons; name = name' } ->
      ...
      propagate_result result (arm_path_constraints @ e_constraints)
      @@ fun result ->
      ...
      Ok (add_conc_info_m m r_symb ~constraints r_concrete)
    | _ ->
      Message.raise_spanned_error (Expr.pos e)
  ...
  | EIfThenElse { cond; etrue; efalse } -> (
    ...
    propagate_result (evaluate_expr ctx lang cond) []
    @@ fun cond ->
    propagate_empty_error_result cond
    @@ fun cond ->
    ...
    | ELit (LBool true) ->
      ...
      in
      propagate_result etrue (c_path_constraint :: c_constraints)
      @@ fun etrue ->
      ...
      Ok (add_conc_info_m e_mark e_symb ~constraints e_concr)
    | ELit (LBool false) ->
      ...
      in
      propagate_result efalse (not_c_path_constraint :: c_constraints)
      @@ fun efalse ->
      ...
      Error
        {
          ...
          constraints = get_constraints result;
        }
    | e -> Ok e
  ...
  | EDefault { excepts; just; cons } -> (
    ...
    | 0 -> (
      ...
      propagate_result just exc_constraints
      @@ fun just ->
      ...
      | EEmptyError ->
        ...
        Ok (add_conc_info_m m None ~constraints EEmptyError)
      | ELit (LBool true) ->
        ...
        in
        Ok (add_conc_info_m c_mark c_symb ~constraints c_concr)
      | ELit (LBool false) ->
        ...
        in
        Message.emit_debug "EDefault>false end";
        Ok (add_conc_info_m m None ~constraints EEmptyError)
      | _ ->
        Message.raise_spanned_error (Expr.pos e)
    ...
    | 1 ->
      ...
      Ok (add_conc_info_e r_symb ~constraints r)
    | _ ->
      Message.raise_multispanned_error
        (List.map
           (fun except ->
...
let eval_conc_with_input
    ...
    (e : yes conc_expr)
    (mark : conc_info mark)
    (fields : yes conc_boxed_expr StructField.Map.t) : 'c conc_result =
  let to_interpret =
    Expr.eapp
      ~f:(Expr.box e)
...
let interpret_program_concolic (type m) (p : (dcalc, m) gexpr program) s :
  ...
  match scope_e with
  | Error _ -> Message.raise_error "error at pre-compilation"
  | Ok ((EAbs { tys = [((TStruct s_in, _) as _targs)]; _ }, mark_e) as scope_e)
    -> begin
    ...
    let rec concolic_loop (path_constraints : annotated_path_constraint list) :
      ...
      | Sat (Some m) -> begin
        ...
        | Error { message; constraints } ->
          print_path_constraints constraints;
          Message.raise_error "Error at evaluation:\n%s" message
        | Ok res ->
          ...
          let outputs_list =
            ...
            | _ ->
              Message.raise_spanned_error (Expr.pos scope_e)
                "The concolic interpretation of a program should always yield \
                 a struct corresponding to the scope variables"
          in
