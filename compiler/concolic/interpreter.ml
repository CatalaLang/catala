...
let make_path_constraint
    (expr : s_expr)
    (pos : Pos.t)
    (branch : bool)
    (complete : bool) : path_constraint =
  { expr; pos; branch; complete }
    ...
    | 0 -> (
      ...
      | ELit (LBool true) ->
        ...
        let j_path_constraint =
          make_path_constraint j_symb (Expr.pos just) true false
        in
      ...
      | ELit (LBool false) ->
        ...
        let not_j_path_constraint =
          make_path_constraint not_j_symb (Expr.pos just) false false
        in
