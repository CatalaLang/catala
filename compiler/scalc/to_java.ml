...
(*   | LInt i -> *)
(* Format.fprintf fmt "integer_of_string(\"%s\")" (Runtime.integer_to_string
   i) *)
...
let format_op (fmt : Format.formatter) (op : operator Mark.pos) : unit =
  ...
  | Add_int_int -> Format.pp_print_string fmt "CatalaInteger.add"
  | Add_rat_rat | Add_mon_mon | Add_dur_dur | Concat ->
    Format.pp_print_string fmt "+"
...
(* let rec format_statement ctx (fmt : Format.formatter) (s : stmt Mark.pos) :
   unit *)
(*     = *)
...
(* Format.fprintf fmt "@[<v 4>if %a is None:@ %a@]@," VarName.format
   switch_var *)
...
let format_typ_lit ppf =
  let open Format in
  function
  | TBool -> fprintf ppf "boolean"
  | TUnit -> fprintf ppf "Void"
...
let format_lit (fmt : Format.formatter) (l : lit Mark.pos) : unit =
  match Mark.remove l with
  | LBool true -> Format.pp_print_string fmt "CatalaBool.true"
  | LBool false -> Format.pp_print_string fmt "CatalaBool.false"
  | LInt i ->
    Format.fprintf fmt "new CatalaInteger(\"%s\")" (Runtime.integer_to_string i)
  | LUnit -> Format.pp_print_string fmt "null"
  | LRat i -> Format.fprintf fmt "new CatalaDecimal(\"%s\")" (Q.to_string i)
  | LMoney e ->
    Format.fprintf fmt "CatalaMoney.from_cents(\"%s\")"
      (Runtime.integer_to_string (Runtime.money_to_cents e))
  | LDate d ->
    Format.fprintf fmt "new CatalaDate(%d,%d,%d)"
      (Runtime.integer_to_int (Runtime.year_of_date d))
      (Runtime.integer_to_int (Runtime.month_number_of_date d))
      (Runtime.integer_to_int (Runtime.day_of_month_of_date d))
  | LDuration d ->
    let years, months, days = Runtime.duration_to_years_months_days d in
    Format.fprintf fmt "new CatalaDuration(%d,%d,%d)" years months days
...
let rec format_expression
  ...
  | EStructFieldAccess { name; field; _ } when StructName.equal name in_struct
    ->
    Format.fprintf fmt "%a" StructField.format field
  | EStructFieldAccess { e1; field; _ } ->
  ...
  | EArray es ->
    Format.fprintf fmt "[%a]"
      (Format.pp_print_list
         ~pp_sep:(fun fmt () -> Format.fprintf fmt ",@ ")
         (fun fmt e -> Format.fprintf fmt "%a" (format_expression jctx ctx) e))
      es
  | ELit l -> Format.fprintf fmt "%a" format_lit (Mark.copy e l)
  | EPosLit ->
    let pos = Mark.get e in
    Format.fprintf fmt
      ...
      format_string_list (Pos.get_law_info pos)
  | EAppOp { op; args = [arg1; arg2]; _ } ->
    Format.fprintf fmt "%a(%a,@ %a)" format_op op
      (format_expression jctx ctx)
      arg1
      (format_expression jctx ctx)
      arg2
  ...
  | EAppOp { op = Log (EndCall, info), _; args = [arg1]; _ }
    ...
    Format.fprintf fmt "log_end_call(%a,@ %a)" format_uid_list info
      ...
      arg1
  | EAppOp { op = Log _, _; args = [arg1]; _ } ->
    Format.fprintf fmt "%a" (format_expression jctx ctx) arg1
  | EAppOp { op = (Not, _) as op; args = [arg1]; _ } ->
    Format.fprintf fmt "%a %a" format_op op (format_expression jctx ctx) arg1
  | EAppOp
      {
        op = ((Minus_int | Minus_rat | Minus_mon | Minus_dur), _) as op;
        args = [arg1];
  ...
  | ETuple es ->
    Format.fprintf fmt "(%a)"
      ...
      es
  | ETupleAccess { e1; index; _ } ->
    Format.fprintf fmt "%a[%d]" (format_expression jctx ctx) e1 index
  | EExternal { modname; name } ->
    Format.fprintf fmt "%a.%s" VarName.format (Mark.remove modname)
      (Mark.remove name)
...
let rec format_stmt ?scope jctx (ctx : Ast.ctx) ppf (stmt : Ast.stmt Mark.pos) =
  ...
  | SSwitch { switch_var; switch_var_typ = _; enum_name; switch_cases } ->
    ...
    let format_switch_case
      ...
      in
      fprintf ppf "@[<v 2>case %a:@ %a%a@]" EnumConstructor.format enum_cstr
        format_init_case
