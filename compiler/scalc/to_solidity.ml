(* This file is part of the Catala compiler, a specification language for tax
   ...
   Denis Merigoux <denis.merigoux@inria.fr>, Jean-Marc Le Roux <contact@jmlx.io>
...
let format_op
  ...
  | Length -> Format.pp_print_string fmt "array_length"
  ...
  | Eq_int_int | Eq_rat_rat | Eq_mon_mon | Eq_dat_dat | Eq_dur_dur ->
    Format.pp_print_string fmt "=="
  | Map -> Format.pp_print_string fmt "array_map"
  | Reduce -> Format.pp_print_string fmt "array_reduce"
  | Filter -> Format.pp_print_string fmt "array_filter"
  | Fold -> Format.pp_print_string fmt "array_fold_left"
...
let avoid_keywords (s : string) : string =
  if
    ...
    | "default" | "define" | "final" | "implements" | "in" | "inline" | "let"
...
let rec format_typ (fmt : Format.formatter) (typ : typ) : unit =
  ...
  in
  let format_func_param_typ (fmt : Format.formatter) (t : typ) =
    ...
    | TLit TDuration
    | TStruct _ ->
      Format.fprintf fmt "%a calldata" format_typ t
    | TLit TUnit -> Format.fprintf fmt ""
    | _ -> Format.fprintf fmt "%a" format_typ t
  in
  ...
  | TArrow (t1, t2) ->
    Format.fprintf fmt "function (%a) returns (%a)"
      (Format.pp_print_list
         ~pp_sep:(fun fmt () -> Format.fprintf fmt ",@ ")
         format_func_param_typ)
      t1 format_func_param_typ t2
...
let format_ctx
    ...
    (fmt : Format.formatter)
    (ctx : decl_ctx) : unit =
  let format_op_eq fmt ((typ : typ), (v : StructField.t)) =
    match Marked.unmark typ with
    | TLit TMoney
    | TLit TInt
    | TLit TRat
    ...
    | TLit TDuration
    | TArray _ | TStruct _ ->
      Format.fprintf fmt "eq(a.%a, b.%a)" format_struct_field_name v
        format_struct_field_name v
    | _ ->
      Format.fprintf fmt "(a.%a == b.%a)" format_struct_field_name v
        format_struct_field_name v
  in
  let format_struct_decl fmt (struct_name, struct_fields) =
    if StructField.Map.is_empty struct_fields then
      (* Solidity does not support structs with 0 field. *)
      Format.fprintf fmt "type %a is bool;" format_struct_name struct_name
    else
      ...
      Format.fprintf fmt
        "@[<hov 4>struct %a {@\n\
         ...
         @\n\
         @[<hov 4>function eq(%a calldata a, %a calldata b)@\n\
         pure@\n\
         returns (bool)@]@\n\
         @[<hov 4>{@\n\
         @[<hov 4>return(@\n\
         ...
         @[<hov 4>for (uint i = 0; i < a.length; i++) {@\n\
         @[<hov 4>if (!eq(a[i], b[i])) {@\n\
         return false;@]@\n\
         }@]@\n\
         }@\n\
        ...
        (Format.pp_print_list
           ...
           (fun fmt (struct_field, struct_field_type) ->
             Format.fprintf fmt "%a %a;" format_typ struct_field_type
               format_struct_field_name struct_field))
        fields format_struct_name struct_name format_struct_name struct_name
        (Format.pp_print_list
           ~pp_sep:(fun fmt () -> Format.fprintf fmt " && @\n")
           (fun fmt (struct_field, struct_field_type) ->
             Format.fprintf fmt "%a" format_op_eq
  ...
  let format_enum_decl fmt (enum_name, enum_cons) =
    if EnumConstructor.Map.is_empty enum_cons then
      ...
      Format.fprintf fmt "@[<hov 4>enum %a {@\n%a@]@\n}" format_enum_name
        enum_name
        (Format.pp_print_list
           ~pp_sep:(fun fmt () -> Format.fprintf fmt ",@\n")
           (fun fmt (enum_cons, enum_cons_type) ->
             Format.fprintf fmt "%a" format_enum_cons_name enum_cons))
        (List.map (fun (x, y) -> x, y) (EnumConstructor.Map.bindings enum_cons))
  in
  ...
  Cli.call_unstyled (fun () ->
      Format.fprintf fmt
        "// This file has been generated by the Catala compiler, do not edit!@\n\
         ...
         function eq(Duration calldata a, Duration calldata b) pure returns \
         (bool) { return(a.value == b.value); }@\n\
         function ne(Duration calldata a, Duration calldata b) pure returns \
         (bool) { return(!eq(a, b)); }@\n\
         @\n\
         struct Money { Integer value; }@\n\
         function eq(Money calldata a, Money calldata b) pure returns (bool) { \
         return(eq(a.value, b.value)); }@\n\
         function ne(Money calldata a, Money calldata b) pure returns (bool) { \
         return(!eq(a, b)); }@\n\
         @\n\
         struct Integer { int256 value; }@\n\
         function eq(Integer calldata a, Integer calldata b) pure returns \
         (bool) { return(a.value == b.value); }@\n\
         function ne(Integer calldata a, Integer calldata b) pure returns \
         (bool) { return(!eq(a, b)); }@\n\
         @\n\
         struct Decimal { fixed256x80 value; }@\n\
         function eq(Decimal calldata a, Decimal calldata b) pure returns \
         (bool) { return(a.value == b.value); }@\n\
         function ne(Decimal calldata a, Decimal calldata b) pure returns \
         (bool) { return(!eq(a, b)); }@\n\
         @\n\
         struct Date { int64 value; }@\n\
         function eq(Date calldata a, Date calldata b) pure returns (bool) { \
         return(a.value == b.value); }@\n\
         function ne(Date calldata a, Date calldata b) pure returns (bool) { \
         return(!eq(a, b)); }@\n\
         @\n\
         %a"
