...
let pp dest fmt =
  Format.kdprintf (fun k -> List.iter k dest) fmt
...
let format_ctx (type_ordering : TypeIdent.t list) ~ppc ~pph ~ppmain (ctx : decl_ctx) :
    unit =
  ...
  List.iter
    (fun struct_or_enum ->
       let pp fmt = pp [ppc; if TypeIdent.Set.mem struct_or_enum ctx.ctx_public_types then pph else ppmain] fmt
       in
      match struct_or_enum with
      | TypeIdent.Struct s ->
        if StructName.path s = [] then (
          let def = StructName.Map.find s ctx.ctx_structs in
          pp "@,%a" format_struct_decl (s, def))
      | TypeIdent.Enum e ->
        if EnumName.path e = [] then (
          let def = EnumName.Map.find e ctx.ctx_enums in
          pp "@,%a" format_enum_decl (e, def)))
...
let format_program
  ...
  File.with_secondary_out_channel ~output_file ~ext:"h" @@ fun h_file pph ->
  File.with_secondary_out_channel ~output_file:(if p.tests = [] then None else output_file) ~ext:"+main.c" @@ fun _ ppmain ->
  ...
  let module_id =
    ...
    | None, None -> "MAIN"
    | None, Some f -> String.uppercase_ascii (String.to_id (File.basename (Filename.remove_extension f)))
    | Some (m, _), _ ->
  ...
  Option.iter (pp [ppmain] "@,#include \"%s\"")
    (Option.map File.basename h_file);

  (* TODO: check the module hash ? *)
  format_ctx type_ordering ~ppc ~pph ~ppmain p.ctx.decl_ctx;
  ppall "@,";
    ...
    List.fold_left
      (fun env code_item ->
        ...
        | SScope
          ...
          in
          let pp_intf = if visibility = Public then [pph] else [ppmain] in
          (* Private scopes are output to the main directly, so that tests can access them ; this prevents us from marking them [static] though *)
          pp (ppc :: pp_intf) "@,@[<v 2>@[<hov 4>";
          if visibility = Private then (pp [ppc] "static "; pp [ppmain] "extern ");
          pp (ppc :: pp_intf) "%a@ @[<hv 1>(%a)@]@]"
            (format_typ ~const:true ctx.decl_ctx (fun fmt ->
                 Format.pp_print_space fmt ();
