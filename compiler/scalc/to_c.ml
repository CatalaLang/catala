...
let format_ctx (type_ordering : TypeIdent.t list) ~ignore_private ~ppc ~pph (ctx : decl_ctx) :
    unit =
    ...
    (fun struct_or_enum ->
      ...
      | TypeIdent.Struct s as tid ->
        if StructName.path s = [] then (
          ...
          if ignore_private || TypeIdent.Set.mem tid ctx.ctx_public_types then
...
let format_main ctx env h_file (fmt : Format.formatter) (p : Ast.program) =
  ...
  Format.fprintf fmt "#include \"%s\"@,@," h_file;
...
let format_program
  ...
  let ignore_private = p.module_name = None in
  (* When not defining a module, everything is exported in the .h file to allow testing *)
  format_ctx type_ordering ~ignore_private ~ppc ~pph p.ctx.decl_ctx;
    ...
    List.fold_left
      (fun env code_item ->
        ...
        | SVar { var; expr; typ; visibility } ->
          ...
          let public = ignore_private || visibility = Public in
  ...
  if p.tests <> [] then
    File.with_secondary_out_channel ~output_file ~ext:"+main.c" @@
    fun _ ppf -> format_main ctx env (File.basename (Option.get h_file)) ppf p
