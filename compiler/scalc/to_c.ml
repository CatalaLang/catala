...
let format_ctx (type_ordering : TypeIdent.t list) ~ppc ~pph (ctx : decl_ctx) :
  ...
  List.iter
    (fun struct_or_enum ->
       let ppfs = ppc :: if TypeIdent.Set.mem struct_or_enum ctx.ctx_public_types then [pph] else []
       in
...
let rec format_statement
  ...
  | SFatalError { pos_expr; error } ->
    Format.fprintf fmt
      "@,\
       @[<hov 2>catala_error(catala_%s,@ %a,@ 1);@]"
...
let format_program
  ...
  File.with_secondary_out_channel ~output_file ~ext:"h" @@ fun _ pph ->
