...
module Commands = struct
  ...
  let interpret_cmd =
    let f
      ...
      let need_check_trace_assertion =
        check_trace_assertion && List.length ex_scopes == 1
      in
      let catala_file = Global.input_src_file options.Global.input_src in
      let need_check_trace_assertion, trace_format =
        ...
        | None | Some (_, `Stdout) ->
          ...
          if need_check_trace_assertion then
            Message.warning
              "@[<hov>The @{<bold>--check-trace-assertion@} flag is activated \
               for scope @{<bold>%s@} of %a, but no trace file was provided to \
               verify the asserted trace variables.@ Please give a trace file \
               to check them.@]"
        ...
        | Some (_, `FileName filename) ->
          ...
          in
          need_check_trace_assertion, trace_format
      in
      ...
      let need_check_trace_assertion =
        ...
        | Global.Human ->
          if need_check_trace_assertion then
            Message.warning
              "@[<hov>The @{<bold>--check-trace-assertion@} flag is activated \
               for scope @{<bold>%s@} of %a, but the trace is not in JSON \
               format.@ Please give a trace file in JSON format to check the \
               asserted trace variables.@]"
              (List.hd ex_scopes) File.format catala_file;
          false
        | Global.JSON -> need_check_trace_assertion
      in
      let () =
        if not lcalc then
      ...
      if need_check_trace_assertion then
        ...
        let trace_assertions =
          Trace_assertion.check
            ~asserted_trace_variables:item.asserted_trace_variables
            ~tested_scope:(List.hd ex_scopes)
            (Trace_assertion.read_trace trace_file)
        in
        match trace_assertions with
        | [] -> Message.result "Asserted trace variables are valid"
        | failures ->
          let pp ppf () =
            Format.fprintf ppf
              "@[<v 2>Asserted trace variables are not correct:@,%a@]"
              (Format.pp_print_list ~pp_sep:Format.pp_print_cut
                 Trace_assertion.display)
              failures
          in
    ...
    Cmd.v
      ...
      Term.(
        ...
        $ global_options
        $ Cli.Flags.ex_scopes
        $ Cli.Flags.check_trace_assertion
        $ Cli.Flags.lcalc
        $ Cli.Flags.closure_conversion
        $ Cli.Flags.monomorphize_types
        $ Cli.Flags.keep_special_ops
