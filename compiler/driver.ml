...
module Commands = struct
  ...
  let get_scopelist_uids prg (scopes : string list) :
      ScopeName.t list =
    match scopes with
    ...
    | [] ->
      ...
      if test_scopes = [] then
        Message.warning
          "The program defines no test scopes.@ \
Please specify option @{<yellow>--scope@} to explicit what to execute@ or@ mark@ scopes@ in@ the@ program@ with@ the@ @{<cyan>#[test]@}@ attribute.@ The program defines the following scopes:@ @[<hv 4>%a@]"
          (Format.pp_print_list ~pp_sep:Format.pp_print_space
             ScopeName.format)
          (List.filter_map (function KScope n, _ -> Some n | _ -> None) exports)
      else
        Message.debug
          "Will execute the following test scopes:@ %a"
  ...
  let get_output ?ext options output_file =
    ...
    File.get_main_out_channel ~source_file:options.Global.input_src ~output_file ?ext
      ()
  ...
  let scopelang options includes output ex_scopes =
    ...
    get_output_format options output @@ fun _ fmt ->
  ...
  let print_interpretation_results options ?(quiet=false) interpreter prg scope_uid =
    try
  ...
  let interpret_dcalc typed options includes optimize check_invariants quiet ex_scopes
      =
    let prg, _ =
      Passes.dcalc options ~includes ~optimize ~check_invariants ~autotest:false
        ~typed
    ...
    let success =
      List.fold_left
        (fun success scope ->
           print_interpretation_results ~quiet options Interpreter.interpret_program_dcalc
             prg scope
           && success
        )
        true
  ...
  let interpret_lcalc
    ...
    let success =
      List.fold_left
        (fun success scope ->
           print_interpretation_results ~quiet options Interpreter.interpret_program_lcalc
             prg scope
           && success)
        true
        (get_scopelist_uids prg ex_scopes)
      ...
      (Cmd.info "interpret" ~man:Cli.man_base
         ~doc:
           "Runs the interpreter on the Catala program, executing the scopes \
            specified with the $(b,-s) option, or the scopes marked as $(i,#[test]) if absent.")
      Term.(
        const f
        $ Cli.Flags.lcalc
        $ Cli.Flags.closure_conversion
  ...
  let ocaml
    ...
    get_output_format options output @@ fun output_file fmt ->
  ...
  let python
    ...
    @@ fun _output_file fmt -> Scalc.To_python.format_program fmt prg type_ordering
  ...
  let c options includes output optimize check_invariants autotest =
    ...
    get_fmt ~ext:"c" @@ fun output_file ppf_src ->
    (if prg.module_name <> None && output_file <> None then
       get_fmt ~ext:"h"
     else
       (fun f -> f None (Format.make_formatter (fun _ _ _ -> ()) ignore)))
