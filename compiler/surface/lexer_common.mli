...
(** Checks that the character immediately following the last lexeme is
    whitespace (or EOF), and raises a lexing error if it is. Any non-whitespace
    character — including '$' — is rejected, so 'under condition$1' triggers an
    error. Only whitespace characters (spaces, tabs, newlines, etc.) and EOF are
    allowed. Used after multi-word keywords to enforce that they must be
