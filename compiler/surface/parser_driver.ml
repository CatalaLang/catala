...
let load_source_file ?default_module_name source_file itf_builder =
  ...
  let modname =
    ...
    | None, None ->
      Message.error ~kind:Parsing
        ...
        | _ -> "Module_name")
  in
  let used_modules, intf = itf_builder program in
  {
    ...
    let rec filter (req, (acc : Ast.code_block)) = function
      ...
      | Ast.ModuleUse (mod_use_name, alias) ->
        ( {
          ...
          :: req,
          acc )
      | Ast.CodeBlock (code, _, _) -> req, List.rev_append code acc
    in
    let req, acc = List.fold_left filter ([], []) program.Ast.program_items in
    List.rev req, List.rev acc
  in
