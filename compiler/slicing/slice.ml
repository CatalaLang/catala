...
let rec list_fold_right4 f l1 l2 l3 l4 accu =
  ...
  | (a1::l1, a2::l2, a3::l3, a4::l4) -> 
    f a1 a2 a3 a4 (list_fold_right4 f l1 l2 l3 l4 accu)
  | _ -> assert false
...
fun e1 e2 -> 
  ...
  | ECustom { obj = o1; _}, ECustom { obj = o2; _ } 
    when o1 = o2 -> e1 
  ...
  | EIfThenElse { cond = c1; etrue = t1; efalse = f1 }, 
    EIfThenElse { cond = c2; etrue = t2; efalse = f2 } -> 
      Mark.add m @@ EIfThenElse { 
        cond = join_expr c1 c2; 
        etrue = join_expr t1 t2; 
        efalse = join_expr f1 f2 
      }
  | EStruct { name = n1; fields = f1 }, EStruct { name = n2; fields = f2 } 
    when StructName.equal n1 n2 ->
      let fields = StructField.Map.mapi (
        fun f e -> join_expr e (StructField.Map.find f f2)
      ) f1 in 
  ...
  | EMatch { name = n1; e = e1; cases = c1 }, 
    ...
    when EnumName.equal n1 n2 ->
      let cases = EnumConstructor.Map.mapi (
        fun c e -> join_expr e (EnumConstructor.Map.find c c2)
      ) c1 in 
  ...
  | EDefault { excepts = x1; just = j1; cons = c1 }, 
    ...
    Mark.add m @@ EDefault { 
      excepts = join_expr_list x1 x2; 
      just = join_expr j1 j2; 
      cons = join_expr c1 c2 
    }
  | EPureDefault e1, EPureDefault e2 -> 
    Mark.add m (EPureDefault (join_expr e1 e2))
  | EErrorOnEmpty e1, EErrorOnEmpty e2 -> 
    Mark.add m (EErrorOnEmpty (join_expr e1 e2))
  | _ -> 
    Message.error "@[<v 2>The two expressions cannot be joined\
      @ Expr1 : %a@ Expr2 : %a" 
    Format_trace.expr e1 Format_trace.expr e2

and join_expr_list : type d t. (d, t) gexpr list -> 
  (d, t) gexpr list -> (d, t) gexpr list =
  fun l1 l2 -> List.map2 join_expr l1 l2

let join_ctx ctx1 ctx2 = 
  (Var.Map.union (fun _ v1 v2 -> Some (join_expr v1 v2))) ctx1 ctx2 

let join_ctx_list ctx_list = 
  List.fold_left join_ctx Var.Map.empty ctx_list
...
fun ctx value trace -> 
  ...
  let rec unevaluate_list ~context_closure v_list trace_list = List.split 
    @@ List.map2 (unevaluate_aux ~context_closure) v_list trace_list

  and unevaluate_op ~context_closure v op trargs tys vargs traux = 
    let m = Mark.get v in
    let unevaluate_auxb = unevaluate_aux ~context_closure in
    let unevaluate_listb = unevaluate_list ~context_closure in
    let unevaluate_opb op tys vargs trargs m = 
      let lctxs, args = unevaluate_listb vargs trargs in 
      join_ctx_list lctxs, Mark.add m (EAppOp { op; args; tys })
    in
    match Mark.remove v, fst op, vargs with
    | _, Op.Length, [(EArray vs, m)] -> 
      let vargs = [
        Mark.add m (EArray (List.map (fun v -> mark_hole (Mark.get v)) vs))
      ] in
      unevaluate_opb op tys vargs trargs m 
    | EArray vs, Map, [EAbs _,mf; (EArray _, ma)] ->
      ...
      let f, sliced_vs = List.fold_right 
        (fun app (joined_f, vs) -> match app with 
          ...
          | e -> Message.error "Application expected, got %a" 
            Format_trace.expr e
        ) sliced_apps (mark_hole mf, [])
      in
      enrich_with_ctx_list contexts @@ 
        unevaluate_opb op tys [f; (EArray sliced_vs, ma)] trargs m
    | EArray vs, Map2, [EAbs _,mf; (EArray _, m1); (EArray _, m2)] -> 
      let contexts, sliced_apps = unevaluate_listb vs traux in
      let f, sliced_vs1, sliced_vs2 = List.fold_right 
        (fun app (joined_f, vs1, vs2) -> match app with 
          | EApp {f; args = [v1; v2]; _}, _ -> 
            (join_expr f joined_f), v1::vs1, v2::vs2
          | EHole _, _ -> joined_f, mark_hole m1 :: vs1, mark_hole m2 ::vs2
          | e -> Message.error "Application expected, got %a" 
          Format_trace.expr e
        ) sliced_apps (mark_hole mf, [], [])
      in
      enrich_with_ctx_list contexts @@ unevaluate_opb op tys 
        [f; (EArray sliced_vs1, m1); (EArray sliced_vs2, m2)] trargs m
    | _, Reduce, [_,mf; EAbs _, md; (EArray [], ma) as arr] 
      when List.length traux = 1 -> 
      let tr = List.hd traux in
      ...
      let sliced_default, sliced_arr = match sliced_app with
        ...
        | e -> Message.error "Application expected, got %a" 
          Format_trace.expr e
      in 
      enrich_with_ctx context @@ unevaluate_opb op tys 
        [mark_hole mf; sliced_default; sliced_arr] trargs m 
    | _, Reduce, [EAbs _, mf; _,md; (EArray (_::_), ma) ] -> 
      let context, f, v0, sliced_vs = List.fold_right (
        fun tr (context, joined_f, acc, vs) ->
          match unevaluate_auxb acc tr with
          | ctx, (EApp {f; args = [acc0; v]; _}, _) -> 
            join_ctx context ctx, join_expr joined_f f, acc0, (v::vs)
          | _, (EHole _, m) -> context, joined_f, mark_hole m, 
            ((mark_hole m)::vs)
          | _, e ->  Message.error "Application expected, got %a" 
            Format_trace.expr e
        )
        traux (Var.Map.empty, mark_hole mf, v, [])
      in
      enrich_with_ctx context @@ unevaluate_opb op tys 
        [f; mark_hole md; (EArray (v0::sliced_vs), ma)] trargs m 
    | EArray vs, Filter, [EAbs _,mf; (EArray _),ma] -> 
      (* This case is particular because we have to slice each element of 
         the array depending on how they behave with the filter function 
         and then join the values with the sliced result. In this case, 
         traux contains alternatively the result of the filter function 
         on an element and the trace of the application *)
      ...
      let bools = List.map (function 
        | Trace_ast.TrLit (LBool b) -> b 
        | tb -> Message.error "Bool was expected, got : %a" 
          Format_trace.trace tb
        ) 
        bools 
      in
      let contexts, sliced_apps = unevaluate_listb (
        List.map (fun b -> ELit(LBool b),m) bools
      ) trbs in
      let f, sliced_arr = List.fold_right 
        (fun app (joined_f, vs) -> match app with 
          | EApp {f; args = [v]; _}, _ -> 
            (join_expr f joined_f), v::vs
          | EHole _, _ -> joined_f, mark_hole ma :: vs
          | _ -> assert false
        )
        sliced_apps (mark_hole mf, [])
      ...
      let rec join_relevant_values bools raw filtered = 
        ...
        | [], [], [] -> []
        | false::bs, r::rs, _ -> 
          r::(join_relevant_values bs rs filtered)
        | true::bs, r::rs, f::fs -> 
          (join_expr r f) :: (join_relevant_values bs rs fs)
        | _ -> assert false
      in 
      let sliced_vs = join_relevant_values bools sliced_arr vs in
      enrich_with_ctx_list contexts @@ unevaluate_opb op tys 
        [f; (EArray sliced_vs, ma)] trargs m
    | _, Fold, [EAbs _, mf; _; (EArray _, ma)] -> 
      let context, f, v0, sliced_vs = List.fold_right (
        fun tr (context, joined_f, acc, vs) ->
          match unevaluate_auxb acc tr with
          | ctx, (EApp {f; args = [acc0; v]; _}, _) -> 
            join_ctx context ctx, join_expr joined_f f, acc0, (v::vs)
          | _, (EHole _, m) -> 
            context, joined_f, mark_hole m, ((mark_hole m)::vs)
          | _, e -> Message.error "Application expected, got %a" 
            Format_trace.expr e
        )
        traux (Var.Map.empty, mark_hole mf, v, [])
      in
      enrich_with_ctx context @@ unevaluate_opb op tys 
        [f; v0; (EArray sliced_vs, ma)] trargs m
    | _ -> unevaluate_opb op tys vargs trargs m
  
  and unevaluate_aux :
    context_closure : (((yes, yes, yes) slicing_interpr_kind, t) gexpr, 
    ((yes, yes, yes) slicing_interpr_kind, t) gexpr) Var.Map.t ->
    ((yes, yes, yes) slicing_interpr_kind, t) gexpr ->
    ((yes, yes, yes) slicing_interpr_kind, t) Trace_ast.t ->
    (((yes, yes, yes) slicing_interpr_kind, t) gexpr, 
    ((yes, yes, yes) slicing_interpr_kind, t) gexpr) Var.Map.t * 
  ...
  fun ~context_closure v trace ->
    ...
    | _, TrExpr _ -> Message.error "This case should not happen,\
      TrExpr cannot be unevaluated"
    ...
    | EAbs {tys = t1; _}, TrAbs {binder; pos; tys= t2; _} 
      ...
      (* There may be variables in the body of the abstraction that have 
         been substituted so to unevaluate the body properly, we have to 
         return the original binder (the one in the trace). If further 
         context is needed, it should have been handled by a TrContextClosure *)
      let original_e = Mark.add m @@ EAbs{binder; pos; tys = t2} in
      if is_sub_expr v original_e then
        (*if the value is a sub expression of the original expression, then 
          the content of the abstraction has been sliced in v so we return it*)
        Var.Map.empty, v
      else
        (* Otherwise, the value contains substitutions 
        ...
        (fun v -> match Var.Map.find_opt v context_closure with 
          ...
          | None -> (
            ...
            (*if the variable is not in the context, 
              then it was not used in the body hence we can assign it a hole*)
            | None -> mark_hole m 
          )
        ) 
        (Array.to_list vars)
      in
      let lctx2, args = unevaluate_listb values trargs in
      let empty_pos = List.init (List.length tys) (fun _ -> Pos.void) in
      let lctx1, f = unevaluate_auxb (Mark.add m (
        EAbs { binder = bind vars e; pos =  empty_pos; tys}
      )) trf in
      join_ctx_list (local_ctx::lctx1::lctx2), Mark.add m (EApp {f; args; tys}) 
    ...
    | _, TrAppOp {op; trargs; tys; vargs; traux} -> 
      unevaluate_op ~context_closure v op trargs tys vargs traux
    | _, TrStructAccess { name; tr; field } ->
      let fields_typ = get_fields ctx name in 
      let fields = StructField.Map.mapi (
        fun f ty -> if f = field then v else (Mark.add m (EHole ty))
      ) fields_typ in
    ...
    | EStruct { name = n1; fields = efields }, 
      TrStruct { name = n2; fields = trfields } 
      when StructName.equal n1 n2 ->
        let fields_with_ctx = StructField.Map.mapi (
          fun f e -> unevaluate_auxb e (StructField.Map.find f trfields)
        ) efields in
        let local_ctx = StructField.Map.fold (
          fun _ (ctx,_) lctx -> join_ctx lctx ctx
        ) fields_with_ctx Var.Map.empty in
    ...
    | _, TrTupleAccess { tr; index; size } -> 
      let arr = List.init size (
        fun i -> if i = index then v else mark_hole m
      ) in
    ...
    | _, TrMatch { name; tr; cases } -> 
      let interesting_cases = EnumConstructor.Map.filter (
        fun _ -> function (Trace_ast.TrExpr _ |TrHole _) -> false | _ -> true 
      ) cases in
      ...
      | _ -> Message.error "Could not determine the case the value\
        was computed from in the match sequence"
      )
    | _, TrIfThenElse { trcond; trtrue; trfalse } -> (
      match trtrue, trfalse with 
        | _, (TrExpr _ |TrHole _)-> (*take the true branch*)
          ...
          join_ctx lctxc lctxt, Mark.add m 
            @@ EIfThenElse {cond; etrue; efalse = mark_hole m}
        | (TrExpr _ |TrHole _), _ -> (*take the false branch*)
          ...
          (join_ctx lctxc lctxf), Mark.add m 
            @@ EIfThenElse {cond; etrue = mark_hole m; efalse}
        | _ -> Message.error "Could not identify whether the result\
          of the condition was true or false" 
      )
    ...
    | _, TrDefault { trexcepts; vexcepts; trjust; trcons } -> (
      ...
      | (TrExpr _ |TrHole _), _ -> 
        (* The result is obtained from one of the exceptions *)
        let lctxs,excepts = unevaluate_listb vexcepts trexcepts in
        let local_ctx = join_ctx_list lctxs in 
        local_ctx, Mark.add m 
          @@ EDefault { excepts; just = mark_hole m; cons = mark_hole m}
      | _, (TrExpr _ |TrHole _) -> 
        (* The result is obtained from the false justification *)
        ...
        local_ctx, Mark.add m 
          @@ EDefault { excepts; just; cons = mark_hole m }
    ...
    | EFatalError err1, TrFatalError { err = err2; tr } when err1 = err2 -> (
      ...
      | Conflict, TrDefault { trexcepts; vexcepts; trjust = _; trcons = _ } -> 
        let lctxs,excepts = List.split(List.map2 (
            fun v tr -> 
              if Mark.remove v = EEmpty then Var.Map.empty, mark_hole m 
              else unevaluate_auxb v tr
          ) vexcepts trexcepts
        ) in
        let local_ctx = join_ctx_list lctxs in 
        local_ctx, Mark.add m 
          @@ EDefault { excepts; just = mark_hole m; cons = mark_hole m}
      | DivisionByZero, TrAppOp { 
          op = (Div_int_int|Div_rat_rat|Div_mon_mon|
            Div_mon_int|Div_mon_rat|Div_dur_dur),_ as op; 
          trargs = [_; _] as trargs; tys; vargs = [_; _] as vargs; _ 
        } ->
        let lctxs, args = unevaluate_listb vargs trargs in
        join_ctx_list lctxs, Mark.add m @@ EAppOp {op; args; tys}

      | NotSameLength, TrAppOp {
          op = (Map2, _) as op; trargs; tys; 
          vargs = [_, mf; EArray vs1, m1; EArray vs2, m2]; _
        } -> 
        let hole_list lst = List.map (fun (_,m) -> mark_hole m) lst in
        let vargs = [
          mark_hole mf; EArray (hole_list vs1), m1; EArray (hole_list vs2), m2
        ] in
        let lctxs, args = unevaluate_listb vargs trargs in
        join_ctx_list lctxs, Mark.add m @@ EAppOp {op; args; tys}

      | UncomparableDurations, TrAppOp { 
          op = (Lt_dur_dur|Lte_dur_dur|Gt_dur_dur
          |Gte_dur_dur|Eq_dur_dur),_ as op; 
          trargs = [_; _] as trargs; tys; 
          vargs = [_; _] as vargs; _ 
        } ->
        let lctxs, args = unevaluate_listb vargs trargs in
        join_ctx_list lctxs, Mark.add m @@ EAppOp {op; args; tys}
      | AmbiguousDateRounding, TrAppOp { 
          op = (Add_dat_dur _|Sub_dat_dur _), _ as op;
          trargs = [_; _] as trargs; tys; vargs = [_; _] as vargs; _ 
        } ->
        let lctxs, args = unevaluate_listb vargs trargs in
        join_ctx_list lctxs, Mark.add m @@ EAppOp {op; args; tys}
      | IndivisibleDurations, TrAppOp { 
          op = Div_dur_dur, _ as op; 
          trargs = [_; _] as trargs; tys; vargs = [_; _] as vargs; _ 
        } ->
        let lctxs, args = unevaluate_listb vargs trargs in
        join_ctx_list lctxs, Mark.add m @@ EAppOp {op; args; tys}
      | _ -> Message.error "The %a in the execution could not be handled by the\ 
        unevaluation function@. Trace : %a" Format_trace.expr v Format_trace.trace trace
    )
    | _ -> Message.error "@[<v 2>The trace does not match the value@\
     Expr : %a@ Trace : %a@]" Format_trace.expr v Format_trace.trace trace
    ...
    | (EAbs { tys = [((TStruct s_in, _) as _targs)]; _ }, mark_e) as e ->
      ...
      if not @@ is_sub_expr uneval_e (addholes e) then 
        Message.error "%a@ Input expression : %a@ Sliced expression : %a" 
          Format.pp_print_text"The sliced expression is not a subexpression \
