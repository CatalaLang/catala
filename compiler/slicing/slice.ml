...
let del_useless_declarations e =
  let rec f :
      ...
      ((d, c, yes) slicing_interpr_kind, 't) gexpr boxed = function
    | EApp { f = EAbs { binder; pos; tys }, mf; args; _ }, m ->
      let vars, body = Bindlib.unmbind binder in
      let body = f body in
      let useful_vars, pos, tys, args =
        list_fold_right4
          (fun var p t e (vars, pos, tys, args) ->
...
let rec join_expr : type d t. (d, t) gexpr -> (d, t) gexpr -> (d, t) gexpr =
 fun e1 e2 ->
  ...
  | ECustom { obj = o1; _ }, ECustom { obj = o2; _ } when o1 = o2 -> e1
...
and join_expr_list :
    type d t. (d, t) gexpr list -> (d, t) gexpr list -> (d, t) gexpr list =
...
let unevaluate :
    ...
    ((yes, yes, yes) slicing_interpr_kind, t) gexpr =
 fun ctx value trace ->
  let rec unevaluate_list ~context_closure v_list trace_list =
  ...
  and unevaluate_op ~context_closure v op trargs tys vargs traux =
      ...
      let f, sliced_vs =
        List.fold_right
          (fun app (joined_f, vs) ->
            ...
            | e ->
              Message.error "Application expected, got %a" Format_trace.expr e)
          sliced_apps
          (mark_hole mf, [])
      in
    ...
    | EArray vs, Map2, [(EAbs _, mf); (EArray _, m1); (EArray _, m2)] ->
      let contexts, sliced_apps = unevaluate_listb vs traux in
      let f, sliced_vs1, sliced_vs2 =
        List.fold_right
          (fun app (joined_f, vs1, vs2) ->
            match app with
      ...
      in
      enrich_with_ctx_list contexts
      @@ unevaluate_opb op tys
           [f; EArray sliced_vs1, m1; EArray sliced_vs2, m2]
           trargs m
    | _, Reduce, [(_, mf); (EAbs _, md); ((EArray [], ma) as arr)]
      ...
      let sliced_default, sliced_arr =
        match sliced_app with
      ...
      in
      enrich_with_ctx context
      @@ unevaluate_opb op tys
           [mark_hole mf; sliced_default; sliced_arr]
           trargs m
    | _, Reduce, [(EAbs _, mf); (_, md); (EArray (_ :: _), ma)] ->
      let context, f, v0, sliced_vs =
        List.fold_right
          (fun tr (context, joined_f, acc, vs) ->
            ...
            | _, e ->
              Message.error "Application expected, got %a" Format_trace.expr e)
          traux
          (Var.Map.empty, mark_hole mf, v, [])
      in
      enrich_with_ctx context
      @@ unevaluate_opb op tys
           [f; mark_hole md; EArray (v0 :: sliced_vs), ma]
           trargs m
    | EArray vs, Filter, [(EAbs _, mf); (EArray _, ma)] ->
      (* This case is particular because we have to slice each element of the
         array depending on how they behave with the filter function and then
         join the values with the sliced result. In this case, traux contains
         alternatively the result of the filter function on an element and the
      ...
      let bools =
        List.map
          (function
            ...
            | tb ->
              Message.error "Bool was expected, got : %a" Format_trace.trace tb)
          bools
      in
      let contexts, sliced_apps =
        unevaluate_listb (List.map (fun b -> ELit (LBool b), m) bools) trbs
      in
      let f, sliced_arr =
        List.fold_right
          (fun app (joined_f, vs) ->
  ...
  and unevaluate_aux :
   ...
   fun ~context_closure v trace ->
    ...
    | _, TrExpr _ ->
      Message.error "This case should not happen,TrExpr cannot be unevaluated"
    ...
    | _, TrApp { trf; trargs; tys; vars; trv } ->
      ...
      let values =
        List.map
          (fun v ->
            match Var.Map.find_opt v context_closure with
            ...
            | None -> (
              ...
              | Some value -> value
              (*if the variable is not in the context, then it was not used in
                the body hence we can assign it a hole*)
              | None -> mark_hole m))
          (Array.to_list vars)
      in
      ...
      let lctx1, f =
        unevaluate_auxb
          (Mark.add m (EAbs { binder = bind vars e; pos = empty_pos; tys }))
          trf
      in
      ( join_ctx_list (local_ctx :: lctx1 :: lctx2),
        Mark.add m (EApp { f; args; tys }) )
    | _, TrAppCustom { trcustom; custom; trargs; vargs; tys; _ } ->
      ...
      ( join_ctx_list (lctx1 :: lctx2),
        Mark.add m (EApp { f = e1; args = e2; tys }) )
    ...
    | _, TrTupleAccess { tr; index; size } ->
      let arr =
        List.init size (fun i -> if i = index then v else mark_hole m)
      in
    ...
    | _, TrMatch { name; tr; cases } -> (
      ...
      | 0 ->
        Message.error
          "No case found to unevaluate the value in the match sequence"
      | 1 -> (
        ...
        | EApp { f = ec; args = [v']; tys = _ } ->
          let lctx, e =
            unevaluate_auxb (Mark.add m (EInj { name; e = v'; cons })) tr
          in
          let cases_expr =
            EnumConstructor.Map.mapi
          ...
          in
          ( join_ctx lctx lctxc,
            Mark.add m (EMatch { name; e; cases = cases_expr }) )
        | _ -> Message.error "Should not happen if well typed")
      | _ ->
        Message.error
          "Could not determine the case the valuewas computed from in the \
           match sequence")
    | _, TrIfThenElse { trcond; trtrue; trfalse } -> (
      ...
      | _, (TrExpr _ | TrHole _) ->
        ...
        let lctxc, cond =
          unevaluate_auxb (Mark.add m (ELit (LBool true))) trcond
        in
        let lctxt, etrue = unevaluate_auxb v trtrue in
        ( join_ctx lctxc lctxt,
          Mark.add m @@ EIfThenElse { cond; etrue; efalse = mark_hole m } )
      | (TrExpr _ | TrHole _), _ ->
        (*take the false branch*)
        let lctxc, cond =
          unevaluate_auxb (Mark.add m (ELit (LBool false))) trcond
        in
        let lctxf, efalse = unevaluate_auxb v trfalse in
        ( join_ctx lctxc lctxf,
          Mark.add m @@ EIfThenElse { cond; etrue = mark_hole m; efalse } )
      | _ ->
        Message.error
          "Could not identify whether the resultof the condition was true or \
           false")
    ...
    | _, TrDefault { trexcepts; vexcepts; trjust; trcons } -> (
      ...
      | _, (TrExpr _ | TrHole _) ->
        ...
        let eempty =
          List.init (List.length trexcepts) (fun _ -> Mark.add m EEmpty)
        in
        let lctxe, excepts = unevaluate_listb eempty trexcepts in
        let lctxj, just =
          unevaluate_auxb (Mark.add m (ELit (LBool false))) trjust
        in
        let local_ctx = join_ctx_list (lctxj :: lctxe) in
        local_ctx, Mark.add m @@ EDefault { excepts; just; cons = mark_hole m }
      | _, _ ->
        (* The result is obtained from the consequence *)
        let eempty =
          List.init (List.length trexcepts) (fun _ -> Mark.add m EEmpty)
        in
        let lctxe, excepts = unevaluate_listb eempty trexcepts in
        let lctxj, just =
          unevaluate_auxb (Mark.add m (ELit (LBool true))) trjust
        in
    ...
    | EFatalError err1, TrFatalError { err = err2; tr } when err1 = err2 -> (
      ...
      | AssertionFailed, TrAssert tr ->
        let local_ctx, e =
          unevaluate_auxb (Mark.add m (ELit (LBool false))) tr
        in
        local_ctx, Mark.add m (EAssert e)
          ...
          TrAppOp
            {
              ...
              trargs;
              tys;
              vargs = [(_, mf); (EArray vs1, m1); (EArray vs2, m2)];
              _;
            } ) ->
      ...
      | ( UncomparableDurations,
          TrAppOp
            {
              op =
                ( ( Lt_dur_dur | Lte_dur_dur | Gt_dur_dur | Gte_dur_dur
                  | Eq_dur_dur ),
                  _ ) as op;
              trargs = [_; _] as trargs;
              tys;
      ...
      | _ ->
        Message.error
          "The %a in the execution could not be handled by the \n\
          \        unevaluation function@. Trace : %a" Format_trace.expr v
          Format_trace.trace trace)
    | _ ->
      Message.error
        "@[<v 2>The trace does not match the value@Expr : %a@ Trace : %a@]"
        Format_trace.expr v Format_trace.trace trace
  in

  snd (unevaluate_aux ~context_closure:Var.Map.empty value trace)

let slice_bench (p : (dcalc, 'm) gexpr program) (s : ScopeName.t) =
  let time_it f =
    let now = Ptime_clock.now () in
    let o, s = f () in
    let delta = Ptime.diff (Ptime_clock.now ()) now in
    delta, o, s
  in
...
let slice ?(debug = false) (p : (dcalc, 'm) gexpr program) (s : ScopeName.t) =
  Message.with_delayed_errors (fun () ->
      let ctx = p.decl_ctx in
      let e = Expr.unbox (Program.to_expr p s) in
      match
        Interpreter.evaluate_expr p.decl_ctx p.lang (Interpreter.addcustom e)
      with
      | (EAbs { tys = [((TStruct s_in, _) as _targs)]; _ }, mark_e) as e ->
        begin
        (* Get the term to interpret *)
...
let test ?(debug = false) (p : (dcalc, 'm) gexpr program) (s : ScopeName.t) =
  let v, sliced_e = slice ~debug p s in
  let value, _ = Interpret.evaluate_expr_safe p.decl_ctx p.lang sliced_e in
  (*if debug then ( Message.log "Result from sliced program :";
    Format.print_newline(); Format_trace.print_expr value;
    Format.print_newline(); Message.log "Trace of sliced program :";
    Format.print_newline(); Format_trace.print_trace trace;
    Format.print_newline() );*)
  Expr.equal v value
