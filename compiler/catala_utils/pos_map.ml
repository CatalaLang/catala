...
let get_value (pmap : 'a gen) pos =
  let name = Pos.get_file pos in
  match File.Map.find_opt name pmap with
  | None -> None
  | Some itvmap ->
    let itv = from_pos pos in
    Interval_map.find_opt itv itvmap

type simple_coverage = Reach | Pos | Neg | Fulf

let get_value_simple (pmap : simple) pos : simple_coverage option =
  match get_value pmap pos with
  | None -> None
  | Some x ->
    Some
      (match x with
...
type 'a origin_ops = { merge : 'a -> 'a -> 'a; eq : 'a -> 'a -> bool }
...
type interval_coverage = Smaller | Equal | Larger

let merge ~ops ~inside x y =
  match x, y with
  | Fulfilled set, (Fulfilled set' | Positive set') ->
    Fulfilled (ops.merge set set')
  | (Fulfilled _ as f), Negative -> if inside = Smaller then Negative else f
  ...
  | Negative, (Positive set | Fulfilled set) -> begin
    match inside with Equal | Smaller -> Fulfilled set | Larger -> Negative
  end
  ...
  | Some (mi, prev) ->
    if i = mi then Interval_map.add i (merge ~inside:Equal prev v) pos_map
      ...
      if i.stop = mi.start && c_equal ops prev v then
        ...
        i.start = mi.start
        && i.stop < mi.stop
        && merge ~inside:Smaller prev v = prev
      then pos_map
      else if i.stop < mi.stop then begin
        (* xxx|------------| |------| *)
        if i.start = mi.start then
          pos_map
          |> Interval_map.remove mi
      ...
      (* |------------------| |---| *)
    else if i.stop <= mi.stop && merge ~inside:Smaller prev v = prev then
      pos_map
    else
      (* |------------| |---???? *)
      let pos_map =
      ...
      if i.stop > mi.stop then
        ...
        let pos_map =
          Interval_map.add i (merge ~inside:Smaller prev v) pos_map
        in
...
let set_ops = { merge = String.Set.union; eq = String.Set.equal }
let unit_ops = { merge = (fun () () -> ()); eq = (fun () () -> true) }

let fusion x y =
  File.Map.union
    (fun _s l r ->
      Some (Interval_map.fold (add_interval ~ops:set_ops ~inside:true) r l))
    x y
...
let simplify_map m =
  let reached_set (loc, c) =
    ( loc,
      match c with
      | Positive set | Fulfilled set -> set
      | Negative | Reachable -> String.Set.empty )
  in
  List.of_seq
  @@ Seq.map reached_set
  @@ Interval_map.to_seq
