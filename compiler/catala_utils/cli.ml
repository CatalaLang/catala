...
module Flags = struct
  ...
  module Global = struct
    ...
    let bin_dir =
      ...
      & info ["bin"] ~docv:"DIR"
          ~doc:
            "Directory containing compiled artifacts. This is used to load shared modules. Defaults to the value of $(b,--directory) if that is set, or to $(b,_build) otherwise, consistently with the defaults used by clerk to create these files."
    ...
    let flags =
      let make
        ...
        let trace, trace_format =
          ...
          | Some (`FileName f), None ->
            ...
            ( Some
              ...
              Some trace_format )
        in
        let bin_dir = match bin_dir with
          | Some d -> path_rewrite d
          | None -> match directory with Some _ -> Filename.current_dir_name | None -> "_build"
        in
        ...
        Global.enforce_options ~language ~debug ~color ~message_format ~trace
          ...
          ~path_rewrite ~stop_on_error ~no_fail_on_assert ~whole_program ~bin_dir ()
      in
      Term.(
        const make
        $ language
