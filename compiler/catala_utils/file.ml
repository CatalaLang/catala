...
module Path = struct
  ...
  let concat ~win32 a b =
    ...
    if l = 0 || is_dir_sep ~win32 a.[l - 1] then a ^ b
    else a ^ dir_sep ~win32 ^ b
  ...
  (* Removes redundant "." segments, folds ".." when possible (keeping extra
     leading ".."), preserves absolute/relative and the drive/root syntax.
     Purely lexical: no symlink or case resolution. *)
...
let path_to_list p = Path.path_to_list ~win32:Sys.win32 p
let clean_path p = Path.clean_path ~win32:Sys.win32 p
...
let reverse_path ?(from_dir = Sys.getcwd ()) ~to_dir f =
  Path.reverse_path ~win32:Sys.win32 ~cwd:(Sys.getcwd ()) ~from_dir ~to_dir f
