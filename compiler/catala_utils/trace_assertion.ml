...
let add_asserted_trace_variable name value map = M.add name value map
...
(* Alternative lookup, kept for reference: instead of following an explicit JSON
   path, it searches the trace by element name. Not wired into
   [check] yet. *)
...
let read_trace (file : File.t) : Yojson.Safe.t option =
  ...
  with Yojson.Json_error msg ->
    Message.warning
      "Invalid JSON in the trace file %a:@ %s@.Disabling check-trace-assertion"
      File.format file msg;
    None

let check ~asserted_trace_variables ~tested_scope (trace : Yojson.Safe.t option)
    =
  if M.is_empty asserted_trace_variables || Option.is_none trace then (
    Message.debug "No asserted trace variable to check for %s" tested_scope;
    [])
  else
    let trace_elements =
      match trace with
    ...
    in
    let testing_scope_var = String.to_snake_case testing_scope in
    ...
    let failures =
      M.fold
        (fun path (v : asserted_trace_variable) failures ->
          let items =
            tested_scope
            :: testing_scope_var
            :: testing_scope
...
let display ppf (assertion : trace_assertion) =
  let f =
    Format.fprintf ppf
      "@[<v 4>@{<red>■@} Variable %s:@,Expected: %s@,Actual: %s@]"
      assertion.name assertion.expected
  in
  match assertion.current_value with None -> f "Missing" | Some l -> f l
