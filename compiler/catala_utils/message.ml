...
let pp_marker ?extra_label target ppf =
  ...
  extra_label |> Option.iter (fun lbl ->
...
module Content = struct
  ...
  let basic_msg ?header ppf target content =
    let pp_header ppf =
      Option.iter (Format.fprintf ppf " %s: ") header
    in
    ...
    Format.pp_print_list
      ...
      (fun ppf -> function
        ...
        | MainMessage msg ->
          ...
          Format.fprintf ppf "@[<hov 2>[%t] %t%t@]" (pp_marker target) pp_header msg
        | Outcome msg ->
          Format.fprintf ppf "@[<hov>[%t]@ %t%t@]" (pp_marker target) pp_header msg
  ...
  let fancy_msg ?header ppf target content =
    ...
    Option.iter (fun h -> Format.fprintf ppf " %s @{<blue>─@}" h)
      header;
...
let with_delayed_errors
  ...
  | Some _ ->
    error ~internal:true "nested call to 'with_delayed_errors'");
  ...
  | Some [], Either.Left result ->
    result
  | Some [err], Either.Left _ ->
    raise (CompilerError err)
  | Some errs, Either.Left _ ->
