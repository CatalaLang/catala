...
let format_scope_exec_args (p : 'm Ast.program) filename (fmt : Format.formatter) exports
    =
  let tests =
    List.filter_map
...
|};
  let modname = match p.module_name with
    | Some (n, _) -> ModuleName.to_string n
    | None -> String.capitalize_ascii (File.basename (Filename.remove_extension filename))
  in
...
let format_program
  ...
  p.module_name |> Option.iter (fun (modname, intf_id) ->
      Format.pp_print_cut fmt ();
  ...
  if List.exists (function KTest _, _ -> true | _ -> false) exports then
    File.with_secondary_out_channel ~output_file ~ext:"+main.ml" (fun _ fmt ->
        format_scope_exec_args p (Option.value ~default:"-" output_file) fmt exports);
  Format.pp_close_box fmt ()
