...
let rec typ_embedding_name (fmt : Format.formatter) (ty : typ) : unit =
  ...
  | TStruct s_name ->
    Format.fprintf fmt "%a%a.embed" Uid.Path.format (StructName.path s_name)
      format_to_module_name (`Sname s_name)
  | TEnum e_name ->
    Format.fprintf fmt "%a%a.embed" Uid.Path.format (EnumName.path e_name)
      format_to_module_name (`Ename e_name)
