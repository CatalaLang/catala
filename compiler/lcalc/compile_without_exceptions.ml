...
and translate_binder (ctx : ctx)
    ((binder, pos_binder) : (D.expr, D.expr Pos.marked) Bindlib.mbinder Pos.marked) :
    (A.expr, A.expr Pos.marked) Bindlib.mbinder Pos.marked Bindlib.box =
...
and translate_expr (ctx : ctx) (e : D.expr Pos.marked) : A.expr Pos.marked Bindlib.box =
  ...
  | D.EVar v ->
      ...
      if info.is_pure then A.make_some info.expr else info.expr
  | D.ETuple (args, s) ->
          ...
          |> List.map (fun (e', _pos) ->
                 ...
                 | _ ->
                     Errors.raise_spanned_error
                       "Internal error: an error occured during the translation of a amtch."
                       (Pos.get_position e))
          |> Bindlib.box_list
        in
  ...
  | D.EOp _op ->
      Errors.raise_spanned_error
        "Internal error: partial application of generic operator are not yet supported when using \
         --avoid_exception."
        (Pos.get_position e)
  | D.EApp ((D.EOp op, pos), args) ->
      let xs =
        List.mapi (fun i arg -> A.Var.make (Printf.sprintf "x_%d" i, Pos.get_position arg)) args
      in
        ...
        |> List.fold_left
             (fun acc (x, arg) ->
               A.make_matchopt arg
                 (A.make_abs
                    (Array.of_list [ dummy ])
                    (A.make_none pos) pos [ (D.TLit D.TUnit, pos) ] pos)
                 (A.make_abs (Array.of_list [ x ]) acc pos [ (D.TAny, pos) ] pos))
             final
      in

      let new_e =
        let+ args_var = xs |> List.map (fun x -> Bindlib.box_var x) |> Bindlib.box_list in

        let args_var =
          args_var |> List.combine args |> List.map (fun (arg, x) -> Pos.same_pos_as x arg)
        in
  ...
  | D.EIfThenElse (e1, e2, e3) ->
      ...
      let new_e new_e1 =
        let+ e1_new = new_e1 and+ e2 = translate_expr ctx e2 and+ e3 = translate_expr ctx e3 in
  ...
  | D.EApp (e1, args) ->
      ...
      let new_e new_e1 =
        let+ new_e1 = new_e1
        and+ args = args |> List.map (translate_expr ctx) |> Bindlib.box_list in
  ...
  | D.EAbs (binder, ts) ->
      let+ new_binder = translate_binder ctx binder in
      same_pos @@ A.make_some' (same_pos @@ A.EAbs (new_binder, List.map translate_typ ts))
...
(* Errors.raise_spanned_error "Internal error: Error on empty found in incorrect place when
   compiling using the --avoid_exception option." (Pos.get_position e) *)
...
let rec translate_scope_vardefinition ctx expr : A.expr Pos.marked Bindlib.box =
  ...
  | expr, pos ->
      Errors.raise_spanned_error
        (Printf.sprintf
           "Internal error: Found unexpected expression when compiling an expression using the \
            --avoid_exception option. ''Full'' term: %s"
           (D.show_expr expr))
        pos
  ...
  | {
   ...
   D.scope_let_expr = expr;
      ...
      let expr' : A.expr Pos.marked Bindlib.box =
        ...
        | ScopeVarDefinition, _typ, expr -> translate_scope_vardefinition ctx expr
        | Assertion, _typ, expr ->
        ...
        | SubScopeVarDefinition, _typ, (D.EAbs ((binder, pos_binder), _tau), pos) ->
            ...
            let ctx =
              ArrayLabels.fold_left vs ~init:ctx ~f:(fun ctx (v : D.expr Bindlib.var) ->
        ...
        | DestructuringInputStruct, _typ, expr -> translate_expr ctx expr
        | DestructuringSubScopeResults, _typ, expr -> translate_expr ctx expr
        | CallingSubScope, _typ, expr -> translate_expr ctx expr
        | kind, _typ, (expr, pos) ->
            let kind_s =
              match kind with
              ...
              | CallingSubScope -> "CallingSubScope"
            in
            ...
            Errors.raise_spanned_error
              (Printf.sprintf
                 "Internal error: Found unexpected %s when compiling an expression containing %s \
                  using the --avoid_exception option. ''Full'' term: %s"
                 kind_s expr_s (D.show_expr expr))
              pos
      in
...
let translate_scope_body (ctx : ctx) (s : D.scope_body) : A.expr Pos.marked Bindlib.box =
  match s with
  | { D.scope_body_lets = lets; D.scope_body_result = result; D.scope_body_arg = arg; _ } ->
...
let translate_program (prgm : D.program) : A.program =
  let new_scopes =
    (prgm.scopes : (D.ScopeName.t * D.expr Bindlib.var * D.scope_body) list)
    |> ListLabels.fold_left ~init:([], D.VarMap.empty) ~f:(fun (acc, ctx) (_, n, e) ->
           let env : ctx =
             D.VarMap.map
               (fun v ->
