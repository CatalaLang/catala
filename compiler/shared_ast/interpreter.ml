...
and val_to_runtime :
 ...
 fun eval_expr ctx ty v ->
  ...
  | TArrow (targs, tret), _ ->
    ...
    curry [] targs
  | TDefault ty, _ -> (
      (* In dcalc, this is an expression.
         in the runtime (lcalc), this is an option(pair(expression, pos)) *)
    match v with
    | EEmpty, _ -> Obj.repr (Runtime.Eoption.ENone ())
    | EPureDefault e, m | (_, m as e) ->
      let e = eval_expr ctx e in
      let pos = Expr.pos e in
      let epos = Obj.magic (EPos pos) in (* EPos is not accepted in dcalc, but here we deal with a lcalc-based runtime *)
      let ty = TTuple [ty; TLit TPos, pos], pos in
      let with_pos =
        ETuple [e; epos, Expr.with_ty m (TLit TPos, pos)],
        (Expr.with_ty m ty)
      in
      Obj.repr (Runtime.Eoption.ESome (val_to_runtime eval_expr ctx ty with_pos))
  )
  | TClosureEnv, v ->
    (* By construction, a closure environment can only be consumed from the same
       scope where it was built (compiled or not) ; for this reason, we can
