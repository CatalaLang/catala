...
let int_encoding : Runtime.runtime_value encoding =
  ...
  union
    [
      case int53
        (function
          | Runtime.Integer z when Z.lt z two53 && Z.gt z (Z.neg two53) ->
            Some (Z.to_int64 z)
          | _ -> None)
...
let runtime_null : Runtime.runtime_value encoding =
  conv (function Runtime.Unit -> () | _ -> assert false) (fun () -> Unit) unit
...
and generate_option_encoder ctx typ =
  ...
  union
    [
      case runtime_null proj_none inj_none;
