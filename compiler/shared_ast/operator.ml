...
let kind_dispatch :
 ...
 fun ~polymorphic ~monomorphic ?(overloaded = fun _ -> assert false)
  ...
  | ( ( Not | GetDay | GetMonth | GetYear | FirstDayOfMonth | LastDayOfMonth
      ...
      _ ) as op ->
    monomorphic op
  | ( ( Log _ | Impossible | Length | Eq | Map | Map2 | Concat | Filter | Reduce
      | Fold | HandleExceptions | FromClosureEnv | ToClosureEnv ),
      _ ) as op ->
    polymorphic op
...
let translate (t : 'a no_overloads t Mark.pos) : 'b no_overloads t Mark.pos =
  match t with
  | ( ( Not | GetDay | GetMonth | GetYear | FirstDayOfMonth | LastDayOfMonth
      | And | Or | Xor | HandleExceptions | Log _ | Impossible | Length | Eq
...
let is_pure : type a. a t -> bool = function
  ...
  | Log _ -> false
  | Not | GetDay | GetMonth | GetYear | FirstDayOfMonth | LastDayOfMonth
  | Impossible | Length | ToClosureEnv | FromClosureEnv | Minus | Minus_int
  | Minus_rat | Minus_mon | Minus_dur | ToInt | ToInt_rat | ToRat | ToRat_int
