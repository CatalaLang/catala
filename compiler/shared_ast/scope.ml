      ...
      (function
        | TArrow (ty_in, ty_out), pos ->
          Expr.make_abs
            ...
            ty_in pos
        | TDefault _, _ as ty ->
          Expr.eempty (Expr.with_ty mark ty)
        | _, pos ->
           Message.error ~pos "%a" Format.pp_print_text
             "Invalid scope for execution or testing: it defines input variables. \
              If necessary, a wrapper scope with explicit inputs to this one can be defined.")
      field_tys
  in
      ...
      (function
        ...
        | TTuple ((TArrow (ty_in, (TOption _, _ as tret)), _) :: _), pos ->
          ...
          Expr.make_tuple
            ...
            mark
        | TOption _, pos as ty -> (* lcalc and later *)
