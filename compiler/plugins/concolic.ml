(* This file is part of the Catala compiler, a specification language for tax
   ...
   Concolic execution extensions Copyright (c) 2023 Rohan Padhye, Carnegie
   Mellon University <rohanpadhye@cmu.edu>
...
let format_lit (fmt : Format.formatter) (l : lit Marked.pos) : unit =
  ...
  | LInt i ->
    Format.fprintf fmt "concolic_integer_of_int(%d)" (Runtime.integer_to_int i)
  | LUnit -> Format.pp_print_string fmt "Unit()"
  | LRat i -> Format.fprintf fmt "concolic_decimal_of_string(\"%a\")" Print.lit (LRat i)
  | LMoney e ->
    Format.fprintf fmt "concolic_money_of_cents_int(%d)"
      (Runtime.integer_to_int (Runtime.money_to_cents e))
  | LDate d ->
    Format.fprintf fmt "concolic_date_of_numbers(%d,%d,%d)"
      (Runtime.integer_to_int (Runtime.year_of_date d))
      (Runtime.integer_to_int (Runtime.month_number_of_date d))
      (Runtime.integer_to_int (Runtime.day_of_month_of_date d))
  | LDuration d ->
    let years, months, days = Runtime.duration_to_years_months_days d in
    Format.fprintf fmt "concolic_duration_of_numbers(%d,%d,%d)" years months days
...
let format_op (fmt : Format.formatter) (op : operator Marked.pos) : unit =
  ...
  | ToRat_int -> Format.pp_print_string fmt "concolic_decimal_of_integer"
  | ToRat_mon -> Format.pp_print_string fmt "concolic_decimal_of_money"
  | ToMoney_rat -> Format.pp_print_string fmt "concolic_money_of_decimal"
  | GetDay -> Format.pp_print_string fmt "concolic_day_of_month_of_date"
  | GetMonth -> Format.pp_print_string fmt "concolic_month_number_of_date"
  | GetYear -> Format.pp_print_string fmt "concolic_year_of_date"
  | FirstDayOfMonth -> Format.pp_print_string fmt "concolic_first_day_of_month"
  | LastDayOfMonth -> Format.pp_print_string fmt "concolic_last_day_of_month"
  | Round_mon -> Format.pp_print_string fmt "concolic_money_round"
  | Round_rat -> Format.pp_print_string fmt "concolic_decimal_round"
...
let rec format_typ (fmt : Format.formatter) (typ : typ) : unit =
  ...
  | TLit TMoney -> Format.fprintf fmt "ConcolicMoney"
  | TLit TInt -> Format.fprintf fmt "ConcolicInteger"
  | TLit TRat -> Format.fprintf fmt "ConcolicDecimal"
  | TLit TDate -> Format.fprintf fmt "ConcolicDate"
  | TLit TDuration -> Format.fprintf fmt "ConcolicDuration"
  | TLit TBool -> Format.fprintf fmt "ConcolicBoolean"
  | TTuple ts ->
       ...
       Format.pp_print_list
         ...
         (fun fmt (struct_field, _) ->
           Format.fprintf fmt "self.%a == other.%a" format_struct_field_name
             struct_field format_struct_field_name struct_field)
      else fun fmt _ -> Format.fprintf fmt "ConcolicBoolean.T")
      fields format_struct_name struct_name
      (Format.pp_print_list
         ~pp_sep:(fun fmt () -> Format.fprintf fmt ",")
         (fun fmt (struct_field, _) ->
  ...
  let format_enum_decl fmt (enum_name, enum_cons) =
    if EnumConstructor.Map.is_empty enum_cons then
      ...
      Format.fprintf fmt
        ...
        \    def __eq__(self, other: object) -> ConcolicBoolean:@\n\
        \        if isinstance(other, %a):@\n\
        \            return self.code == other.code & self.value == \
         other.value@\n\
        \        else:@\n\
        \            return ConcolicBoolean.F@\n\
         @\n\
