...
(* Note: the line number is enough for now, we ignore the rest (recomputing the cnum would require reading the file) *)
...
let run_catala_test_scopes test_flags catala_exe catala_opts filename =
  ...
  let command_ic = Unix.in_channel_of_descr cmd_out_rd in
  let env = catala_test_env () in
  let cmd = Array.of_list (catala_exe :: "interpret" :: "--quiet" :: "--message-format=gnu" :: filename :: catala_opts @ test_flags) in
  let pid =
  ...
  let parse_error line =
    let re_error =
      ...
      compile @@ whole_string @@ seq
        [bos; group ~name:"file" @@ (rep1 (diff any (char ':'))); char ':';
    ...
    | Some g ->
      let gets label = Re.Group.get g (List.assoc label (Re.group_names re_error)) in
      let file = gets "file" in
      let pos = get_pos file in
      let geti label = int_of_string (gets label) in
      Some ((pos (geti "line0") (geti "col0"), pos (geti "line1") (geti "col1")), gets "message")
    | None -> None
  in
  let re_line =
    let open Re in
    ...
    Seq.fold_left
      (fun (errs, acc) line ->
         ...
         | Some g ->
           ...
           let result = match Re.Group.get g 2 with "passed" -> true | "failed" -> false | _ -> assert false in
           [], { Clerk_report.s_name = scope; s_success = result; s_command_line =
                                                       catala_exe :: "interpret" :: filename :: catala_opts @ test_flags @ ["--scope="^scope];
                 s_errors = List.rev errs } :: acc
         | None ->
           match parse_error line with
           | Some (pos, err) -> ((pos, err) :: errs), acc
           | None ->
             Message.debug "Ignored unrecognised output line from 'catala interpret':@ %S" line;
             errs, acc)
      ([], []) out_lines
  in
  let return_code =
