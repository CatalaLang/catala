...
module Spec : Sig.Spec = struct
  ...
  let write_target_def_file ~config:_ ~info ~dir target =
    let open File in
    ...
    if target.tname = Module_graph.stdlib_target_name then (
      Printf.fprintf oc "%s.a %s.so:" target.tname target.tname;
      List.iter
        (fun mname -> Printf.fprintf oc " %s.o" (String.to_id mname))
        ("dates_calc" :: "catala_runtime" :: target.tmodules))
    else (
      Printf.fprintf oc "%s.a:" target.tname;
      List.iter
        (fun mname -> Printf.fprintf oc " %s.o" (String.to_id mname))
        target.tmodules;
      output_string oc "\n";
      Printf.fprintf oc "%s.so:" target.tname;
      List.iter
        (fun mname -> Printf.fprintf oc " %s.o" (String.to_id mname))
        target.tmodules;
      List.iter
        (fun tname -> Printf.fprintf oc " %s.so" tname)
        (List.rev target.dependencies);
      output_string oc "\n")

  let install_extensions config =
    src_extensions
    @ if config.Clerk_cli.include_objects then [obj_extension] else []
    ...
    List.iter
      (fun ext ->
        let src_libcatala =
          ...
          -.- ext
        in
        let libdir = Lazy.force Poll.stdlib_dir / name in
        let src = (libdir / "catala_runtime") -.- ext in
        if File.exists src_libcatala then copy_in ~dir ~src:src_libcatala
        else if File.exists src then copy_in ~dir ~src;
        copy_in ~dir ~src:((libdir / "dates_calc") -.- ext))
      (install_extensions config)

  let write_project_def ~config ~info =
    let open File in
    with_out_channel
      (config.Clerk_cli.file.global.target_dir / name / "Makefile")
    @@ fun oc ->
    Printf.fprintf oc "# Stub Makefile generated by Clerk for %s.\n"
      (Option.value config.Clerk_cli.file.global.project_name
         ~default:"a catala project");
    Printf.fprintf oc
      "# Feel free to replace with your own, using */make.deps files directly\n\n";
    let tgs =
      String.Map.fold
        (fun tname t acc ->
          if
