...
module Spec : Sig.Spec = struct
  ...
  let write_target_def_file ~config:_ ~info ~dir target =
    ...
    List.iter
      ...
      target.tmodules;
    output_string oc "\n";
    Printf.fprintf oc "%s.so:" (String.to_id target.tname);
    List.iter
      (fun mname -> Printf.fprintf oc " %s.o" (String.to_id mname))
      target.tmodules;
    List.iter
      (fun tname -> Printf.fprintf oc " %s.so" (String.to_id tname))
      target.dependencies;
    output_string oc "\n"
  ...
  let write_project_def ~config ~info ~dir =
    ...
    output_string oc
      {make|
INCLUDE = $(CLERK_TARGETS) /home/lg/.opam/default/lib/catala/runtime/c/ # FIXME: copy files and remove this last part
CFLAGS = -fPIC $(INCLUDE:%=-I%)
LDLIBS = -lgmp
