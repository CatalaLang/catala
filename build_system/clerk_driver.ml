  ...
  | `OCaml ->
    ...
    @ List.map
        (fun it ->
          ...
          ^ ".cmx")
        (link_deps item)
    @ [target -.- "cmx"; Filename.remove_extension target ^ "+main.cmx"; "-o"; target -.- "exe"]
  | `C ->
    get_var Var.cc_exe
    @ List.map
        (fun it ->
...
let make_target ~build_dir ~backend item =
  let open File in
  let f =
    Scan.target_file_name item ^ Filename.extension item.Scan.file_name
  in
          ...
          | ".exe" ->
            ...
            let item =
              try
                List.find
                  (fun it ->
                     let item_name = match it.Scan.module_def with
...
let run_tests ~build_dir ~fix_path ~nin_file ~items ~var_bindings ~ninja_flags ~backend ~scope ~cmd files_or_folders =
  let open File in
  let items = List.of_seq items in
  let target_items =
...
let test_cmd =
  let run
    ...
    if backend <> `Interpret then
      if test_flags <> [] then
        ...
        Message.error "@{<bold>--reset@} can only be supplied with the default @{<yellow>interpret@} backend"
      else if xml then
        Message.error "Option @{<bold>--xml@} was specified, but the output of a test report is only supported with the default @{<yellow>interpret@} backend at the moment";
    let enabled_backends =
    ...
    if backend <> `Interpret then
      ...
      let exit_code =
        run_tests ~build_dir ~fix_path ~nin_file ~items ~backend ~ninja_flags ~cmd:"unused" ~scope:None ~var_bindings files_or_folders
      in
      raise (Catala_utils.Cli.Exit_with exit_code)
    else
    let targets, missing =
      let fs = if files_or_folders = [] then ["."] else files_or_folders in
      List.partition_map File.(fun f0 ->
    ...
    | 0 ->
      ...
      if reset_test_outputs then
        let () =
          ...
          match List.filter (fun f -> List.exists (fun t -> not t.i_success) f.tests) reports with
          | [] ->
            Format.fprintf ppf
  ...
  let doc =
    "Scan the given files or directories for catala tests, build their \
     requirement and run them all. If $(b,--backend) is unspecified or $(b,interpret), both scope tests and CLI tests are run ; $(b,--reset) can be used to rewrite the expected results of CLI tests to their current result. For any other $(b,--backend), CLI tests are skipped and scope tests are compiled to the specified backend with the catala option $(b,--autotest), and then run, ensuring the consistency of results."
  in
  Cmd.v (Cmd.info ~doc "test")
    Term.(
      const run
      $ Cli.Global.term (fun ~autotest:_ -> Clerk_rules.ninja_init ~autotest:true)
      $ Cli.files_or_folders
      $ Cli.backend
