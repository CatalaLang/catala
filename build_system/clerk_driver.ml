...
let run_targets
  ...
  | `Interpret ->
    ...
    if test_targets = [] then 0
      ...
      let catala_flags =
        Var.get_var var_bindings (Var.name Var.catala_flags)
        @ (match scope with
          | None -> []
...
let list_vars_cmd =
  let run config =
    let var_bindings =
      Var.env_of_bindings
        (Clerk_rules.base_bindings ~autotest:false ~trace:None
           ~trace_format:None ~code_coverage:false
    ...
    let var_bindings =
      Var.env_of_bindings
        (Clerk_rules.base_bindings ~autotest:false ~code_coverage:false
           ~trace:None ~trace_format:None ~enabled_backends:[] ~config
           ~inplace:true)
    in
