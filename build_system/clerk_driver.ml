...
type backend = OCaml | Python | C (* | JS *)

let all_backends = [ OCaml; Python; C ]
...
let test_cmd =
  let run
    ...
    ninja_init ~enabled_backends:[OCaml] ~extra:Seq.empty ~test_flags
    @@ fun ~build_dir ~fix_path ~nin_file ~items:_ ~var_bindings:_->
    let targets =
  ...
  let run
    ...
    let target_item =
      ...
      Seq.find (fun item ->
        ...
        items
        with Some t -> t
           | None -> Message.error "Source file or module %a not found" File.format file
    in
    let src_file = File.(build_dir / target_item.Scan.file_name) in
    let ninja_cmd = ninja_cmdline ninja_flags nin_file [src_file] in
    ...
    if exit_code = 0 then
      let rec get_var v =
        ...
        List.map
          (Re.replace ~all:true Re.(compile (seq [ str "${"; group (rep1 (diff any (char '}'))); char '}' ]))
             ~f:(fun g -> String.concat " " (get_var (Var.make (Re.Group.get g 1)))))
          s
  (* replace ${var} with get_var v *)
      in
      let catala_flags =
      ...
      in
      let catala_cmd =
        get_var Var.catala_exe @ [ "interpret"; src_file ] @ catala_flags
      in
      raise (Catala_utils.Cli.Exit_with (run_command ~clean_up_env:false catala_cmd))
    else
      raise (Catala_utils.Cli.Exit_with exit_code)
  in
  let doc =
