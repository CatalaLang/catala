...
module Cli = struct
  ...
  let backend =
    Arg.(
      value
      & opt (enum ["interpret", `Interpret; "ocaml", `OCaml; "c", `C; "python", `Python]) `Interpret
      & info ["backend"] ~docv:"BACKEND"
        ~doc:"Run the program using the given backend. $(docv) must be one of $(b,interpret), $(b,ocaml), $(b,c), $(b,python)")
  ...
  let run_command =
    Arg.(
      ...
      & info ["command"] ~docv:"CMD"
        ~doc:"The catala command to run on the input files. Normally $(b,interpret), this flag can be used to run $(b,typecheck) or a custom plugin instead. This is ignored if $(i,--backend) isn't $(b,interpret).")
...
let base_bindings catala_exe catala_flags0 build_dir include_dirs test_flags enabled_backends =
  let includes =
    List.fold_right
      (fun dir flags ->
  ...
  let include_deps =
    Nj.build "copy" ~inputs:[!Var.src]
      ~implicit_in:
        ...
        @ List.map (fun m -> try List.assoc m same_dir_modules with Not_found -> m ^ "@src") modules)
      ~outputs:[catala_src]
  in
  let module_deps =
    Option.map
  ...
  let cc =
    Nj.build "c-object"
      ...
      ~implicit_in:
        (!Var.catala_exe
        :: (!Var.target ^ ".h")
        :: List.map (modfile ".h" ~mod_ext:"@-module") modules)
      ~outputs:[!Var.target ^ ".c.o"]
      ~vars:[Var.includes, [!Var.includes]]
  in
  let expose_module =
    (* Note: these rules define direct (top-level) aliases for module targets of modules that are in include-dirs, so that Ninja can find them from wherever; they are only in implicit-in, because once they are built the compilers will find them independently through their '-I' arguments.

       This works but it might make things simpler to resolve these aliases at the Clerk level ; this would force an initial scan of the included dirs but then we could use the already resolved target files directly and get rid of these aliases.
    *)
    match item.module_def with
    | Some m when List.mem (dirname src) include_dirs ->
      Nj.build "phony" ~outputs:[m ^ "@src"] ~inputs:[!Var.src] ::
      (if List.mem OCaml enabled_backends then
         [Nj.build "phony" ~outputs:[m ^ "@ml-module"] ~inputs:[module_target !Var.target]]
       else []) @
      (if List.mem C enabled_backends then
         [Nj.build "phony"
            ~outputs:[m ^ "@c-module"]
      ...
      if (not item.has_inline_tests) && item.legacy_tests = [] then []
        ...
        [
          Nj.build "tests" ~inputs:[catala_src]
            ~implicit_in:
              (!Var.clerk_exe ::
               List.map (modfile "@ml-module") modules @
...
let gen_ninja_file catala_exe catala_flags build_dir include_dirs test_flags enabled_backends dir
  =
  let var_bindings = base_bindings catala_exe catala_flags build_dir include_dirs test_flags enabled_backends in
  let prologue =
    Seq.return
  ...
  prologue
  @+ Seq.concat main
  @+ (if List.mem Tests enabled_backends then Seq.return (Nj.build "phony" ~outputs:["test"] ~inputs:[".@test"]) else Seq.empty),
  Seq.flat_map List.to_seq items,
  List.filter_map (function Nj.Binding b -> Some b | _ -> None) var_bindings

(** {1 Driver} *)

(* Last argument is a continuation taking as arguments the enabled backends, [build_dir], the
...
let ninja_init
    ...
    (build_dir:File.t -> fix_path:(File.t -> File.t) -> nin_file:File.t -> items:Scan.item Seq.t -> var_bindings:Nj.Binding.t list -> 'a) ->
    'a =
...
let target_backend t =
  let aux = function
    ...
    | ext -> Message.error "Unhandled extension @{<red;bold>%s@}} for target @{<red>%S@}" ext t
  in
  match Filename.extension t with
  | ".exe" -> (match Filename.extension (Filename.chop_extension t) with
      | "" -> OCaml
      | e -> aux e)
  | ext -> aux ext
...
let build_cmd =
  let run ninja_init (targets : string list) (ninja_flags : string list) =
    let enabled_backends =
      targets |> List.map target_backend
    ...
    if targets <> [] then (
      let ninja_targets, _clerk_targets =
        List.partition_map
          (fun t0 ->
             ...
             let t = if String.starts_with ~prefix:(build_dir ^ "/") t then t else File.(build_dir / t) in
             match Filename.extension t with
      ...
      if exit_code = 0 then
        Message.result "@[<v 4>Build successful. The targets can be found in the following files:@,%a@]"
          (Format.pp_print_list
             (fun ppf f ->
                Format.fprintf ppf "@{<cyan>%s@}"
                  (String.remove_prefix ~prefix:File.(original_cwd / "")
                     File.(Sys.getcwd() / f))))
          ninja_targets;
  ...
  in
  let doc =
    "Base build command for individual file targets. Given the corresponding Catala module is declared, this can be used to build .ml, .cmxs, .c, .py files, etc. Targets, along with their dependencies, are always written into $(i,build-dir) (by default $(b,_build)). If a catala extension is used as target, this compiles all its dependencies."
  in
...
let run_cmd =
  let run
      ...
      (scope : string option)
      (ninja_flags : string list) =
    let enabled_backends = match backend with
    ...
    Message.debug "Enabled backends: %a" (Format.pp_print_list ~pp_sep:Format.pp_print_space Format.pp_print_string) (List.map (function
      | OCaml -> "OCaml"
      | C -> "C"
      | Python -> "Python"
      | Tests -> "Tests"
      ...
      List.concat_map (fun file ->
          ...
          let filter item =
            if is_dir then
              ...
              item.Scan.file_name = ffile || Filename.remove_extension item.file_name = ffile
          in
          match List.filter filter items with
          | [] -> Message.error "No source file or module matching %a found" File.format file
          | targets -> targets)
        files_or_folders
    in
    let modules =
      List.fold_left (fun acc it -> match it.Scan.module_def with
    ...
    let rem_dups =
      let rec aux seen = function
        ...
        | [] -> []
      in aux String.Set.empty
    in
    let link_deps item =
      let rec traverse acc item =
        List.fold_left (fun acc m ->
            if List.exists (fun it -> it.Scan.module_def = Some m) acc then acc else
            let it = String.Map.find m modules in
            traverse (it :: acc) it)
          acc (item.Scan.used_modules)
      in
      rem_dups (traverse [] item)
      (* |> fun r -> Message.debug "%a => %a" File.format item.Scan.file_name (Format.pp_print_list ~pp_sep:Format.pp_print_space (fun ppf i -> File.format ppf i.Scan.file_name)) r; r *)
    in
    let make_target item =
      let open File in
      let f = match item.Scan.module_def with
        | None -> item.Scan.file_name
        | Some m -> dirname item.Scan.file_name / m ^ Filename.extension item.Scan.file_name
      in
      let base =
        match backend with
    ...
    in
    let base_targets =
      List.map (fun it -> it, make_target it) target_items
    in
    let multi_targets = match base_targets with _::_::_ -> true | _ -> false in
    let ninja_cmd =
      let ninja_targets =
        List.fold_left (fun acc (it, t) ->
            if String.Set.mem t acc then acc
            else
              String.Set.add t @@
              List.fold_left (fun acc it -> String.Set.add (make_target it) acc) acc (link_deps it))
          String.Set.empty
          base_targets
      in
      ninja_cmdline ("--quiet"::ninja_flags) nin_file (String.Set.elements ninja_targets)
    in
    ...
    let rec get_var v =
      ...
      List.concat_map
        (fun s -> match Re.exec_opt re_single_var s with
           | Some g -> get_var (Var.make (Re.Group.get g 1))
           | None ->
             [Re.replace ~all:true re_var
                ~f:(fun g -> String.concat " " (get_var (Var.make (Re.Group.get g 1))))
                s])
        s
    in
    let run_cmd item target =
      ...
      | `OCaml ->
        get_var Var.ocamlopt_exe @ get_var Var.ocaml_flags @ get_var Var.runtime_ocaml_libs @
        List.map File.(fun it -> build_dir / Filename.dirname it.Scan.file_name / Option.get it.Scan.module_def ^ ".cmx") (link_deps item) @
        [ target; "-o"; File.(target -.- "exe") ]
      | `C ->
        get_var Var.cc_exe @
        List.map File.(fun it -> build_dir / Filename.dirname it.Scan.file_name / Option.get it.Scan.module_def ^ ".c.o") (link_deps item) @
        [ target ] @ get_var Var.c_flags @ ["-o"; File.(target -.- "exe")]
      | `Python ->
        (* TODO: setup the proper environment so that the runtime and modules can be found *)
        [ "python3"; target ]
    in
    let run_artifact src =
      ...
      | `C ->
        let cmd = File.(src -.- "exe") :: Option.to_list scope (* NOTE: not handled yet by the backend *) in
        Message.debug "Executing artifact: '%s'..." (String.concat " " cmd);
        run_command ~clean_up_env:false cmd
      | `Python -> 0
    in
    let cmd_exit_code =
      List.fold_left (fun code (item, target) ->
          if multi_targets then
            Format.fprintf (Message.err_ppf ()) "@{<blue>>@} @{<cyan>%s@}@."
                  (String.remove_prefix ~prefix:File.(original_cwd / "")
                     File.(Sys.getcwd() / String.remove_prefix ~prefix:(build_dir^"/") target -.- ""));
  ...
  let doc =
    "Runs the Catala interpreter on the given files, after building their \
     dependencies. The scope to be executed can be specified using the \
     $(i,-s) option."
  in
...
let main_cmd =
  Cmd.group Cli.info [build_cmd; test_cmd; run_cmd; runtest_cmd; report_cmd; raw_cmd]
