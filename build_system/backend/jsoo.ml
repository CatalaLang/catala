...
module Backend = struct
  ...
  let[@ocamlformat "disable"] static_base_rules backends =
      ...
      Nj.rule "jsoo-bytobject"
        ~command:[
          !Ocaml.ocamlc_exe; "-c"; !Ocaml.ocaml_flags; !jsoo_include; "-I"; runtime_include; !includes; !ppx; !input
        ]
        ~description:["<ocaml>"; "⇒"; !output];
      Nj.rule "jsoo-exe"
        ~command:[
          !Ocaml.ocamlc_exe; !Ocaml.ocaml_flags; !jsoo_include; "-I"; runtime_include; !includes; !ppx; !input; "-o"; !output;
        ]
        ~description:["<ocaml>"; "⇒"; !output];
      Nj.rule "catala-jsoo"
    ...
    if item.Scan.is_stdlib then (
      ...
      let js, missing =
        Ninja.extern_src ~filename:item.Scan.file_name ~backend:"jsoo" ~ext:"js"
          ~missing:[]
      in
      (* Verify if the external implementation is missing, if the implementation is
         missing prints an error message so that the user creates that interface. *)
      Ninja.check_missing ~backend:"jsoo" ~module_def:item.Scan.module_def
        ~missing ~filename:item.file_name;
      List.to_seq
        [
