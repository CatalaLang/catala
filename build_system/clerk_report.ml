...
let disp_flags =
  { files = `Failed; tests = `FailedFile; diffs = true; diff_command = None; fix_path = Fun.id }
...
let clean_command_line ~build_dir file cl =
  ...
  |> (function
      | catala :: cmd :: args ->
        ...
        @
        ("-I" :: pfile ~build_dir (Filename.dirname file) ::
         args)
       | cl -> cl)
  |> function
  | catala :: cmd :: args
...
let pp_pos ~build_dir ppf (start, stop) =
  ...
  Format.fprintf ppf "@{<cyan>%s:%d%a@}"
    ...
    (fun ppf n -> if n = start.Lexing.pos_lnum then () else Format.fprintf ppf "-%d" n)
...
let display ~build_dir file ppf t =
  ...
  if t.i_success then (
    Format.fprintf ppf "@{<green>■@} %a cli test passed" (pp_pos ~build_dir) t.i_expected;
    if Global.options.debug then print_command ~build_dir ppf file t.i_command_line)
  else (
    Format.fprintf ppf "@{<red>■@} %a cli test failed" (pp_pos ~build_dir) t.i_expected;
...
let display_scope ~build_dir file ppf scope_test =
  ...
  if scope_test.s_success then (
    Format.fprintf ppf "@{<green>■@} scope @{<hi_magenta>%s@} passed" scope_test.s_name;
    if Global.options.debug then print_command ~build_dir ppf file scope_test.s_command_line
  ) else (
    Format.fprintf ppf "@{<red>■@} scope @{<hi_magenta>%s@} failed" scope_test.s_name;
    print_command ~build_dir ppf file scope_test.s_command_line;
    List.iter (fun (pos, msg) ->
        Format.fprintf ppf "@,%a %s"
          (pp_pos ~build_dir) pos msg)
      scope_test.s_errors
  );
  Format.pp_close_box ppf ()
    ...
    (fun ppf f ->
      ...
      Format.pp_print_list
        (fun ppf t ->
          ...
          | ((pos, _), _)::_ ->
            Format.fprintf ppf "@[<v 2><testcase name=\"%s\" line=\"%d\">" t.s_name pos.pos_lnum
          | _ ->
            Format.fprintf ppf "@[<v 2><testcase name=\"%s\">" t.s_name
          );
          Format.fprintf ppf
            "@,\
             @[<hv 2><property name=\"description\">@,\
          ...
          if not t.s_success then (
            Format.fprintf ppf
              "@,@[<v 2><failure message=\"Scope failed\">@,";
            Format.pp_print_list (fun ppf (pos, msg) ->
