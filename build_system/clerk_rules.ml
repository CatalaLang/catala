  ...
  let ocaml, c, python, java =
    if item.extrnal then
      ...
      let implicit_out backend ext =
        if has_scope_tests then [target ~backend ("+main." ^ ext)]
        else []
      in
  ...
  let ocamlopt =
    ...
    (if has_scope_tests then [
      Nj.build "ocaml-object"
        ...
        ~outputs:
          (List.map (fun ext -> target ~backend:"ocaml" ("+main." ^ ext)) ocaml_exts)
  ...
  let javac =
    let java_class_path =
      String.concat ":"
        (!Var.tdir / "java"
        :: List.map
             (fun d -> (if Filename.is_relative d then !Var.builddir / d else d) / "java")
             include_dirs)
    in
    Nj.build "java-class"
      ~inputs:[target ~backend:"java" "java"]
      ~implicit_in:
        (List.map (modfile ~backend:"java" ".java") modules)
    ...
    | Some m when List.mem (dirname src) include_dirs ->
      ...
      @ (if List.mem C enabled_backends then
           [
             Nj.build "phony"
               ~outputs:[m ^ "@c-module"]
               ~inputs:[modfile ~backend:"c" ".h" m; modfile ~backend:"c" ".o" m];
             (* Nj.build "phony"
              *   ~outputs:[m ^ "@h-module"]
...
let gen_build_statements_dir
  ...
  let check_conflicts seen item =
    ...
    | Some f1 ->
      Message.error "Conflicting file names:@ %S@ and@ %S@ would both generate the same target file@ %S.@ Please rename one of them."
  ...
  if List.mem Tests enabled_backends then
    let inputs =
      List.rev_append
        ...
        (List.filter_map
           (fun item ->
             if not (item.Scan.has_inline_tests || Lazy.force item.Scan.has_scope_tests) then None
