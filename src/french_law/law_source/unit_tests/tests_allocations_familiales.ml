...
type donnees_out = {
  enfant1_out : enfant_entree;
  enfant2_out : enfant_entree;
  enfant3_out : enfant_entree;
  enfant4_out : enfant_entree;
  enfant5_out : enfant_entree;
}

type donnees_in = {
  enfant1_in : unit -> enfant_entree;
  enfant2_in : unit -> enfant_entree;
  enfant3_in : unit -> enfant_entree;
  enfant4_in : unit -> enfant_entree;
  enfant5_in : unit -> enfant_entree;
}
...
let donnees (donnees_in : donnees_in) =
  let enfant1_ : unit -> enfant_entree = donnees_in.enfant1_in in
  let enfant2_ : unit -> enfant_entree = donnees_in.enfant2_in in
  let enfant3_ : unit -> enfant_entree = donnees_in.enfant3_in in
  let enfant4_ : unit -> enfant_entree = donnees_in.enfant4_in in
  let enfant5_ : unit -> enfant_entree = donnees_in.enfant5_in in
  let enfant1_ : enfant_entree =
    try
      handle_default
        [| (fun (_ : _) -> enfant1_ ()) |]
        (fun (_ : _) -> true)
            ...
            [|
              (fun (_ : _) ->
                handle_default [||]
                  ...
                  (fun (_ : _) ->
                    {
                      d_identifiant = integer_of_string "1";
                      d_remuneration_mensuelle = money_of_cents_string "0";
                      d_date_de_naissance = date_of_numbers 2007 1 1;
    ...
    with EmptyError ->
      raise
        (NoValueProvided
           {
             filename = "tests_allocations_familiales.catala_fr";
             start_line = 9;
             start_column = 12;
             end_line = 9;
             end_column = 19;
             law_headings = [ "Tests"; "Tests" ];
           })
  in
  let enfant2_ : enfant_entree =
    try
      handle_default
        [| (fun (_ : _) -> enfant2_ ()) |]
        (fun (_ : _) -> true)
            ...
            [|
              (fun (_ : _) ->
                handle_default [||]
                  ...
                  (fun (_ : _) ->
                    {
                      d_identifiant = integer_of_string "2";
                      d_remuneration_mensuelle = money_of_cents_string "0";
                      d_date_de_naissance = date_of_numbers 2009 1 1;
                      d_prise_en_charge = EffectiveEtPermanente ();
                      d_a_deja_ouvert_droit_aux_allocations_familiales = true;
                    }));
            |]
            ...
            [|
              (fun (_ : _) ->
                handle_default [||]
                  ...
                  (fun (_ : _) ->
                    {
                      d_identifiant = integer_of_string "3";
                      d_remuneration_mensuelle = money_of_cents_string "40000";
                      d_date_de_naissance = date_of_numbers 2003 1 1;
                      d_prise_en_charge = GardeAlterneePartageAllocations ();
                      d_a_deja_ouvert_droit_aux_allocations_familiales = true;
                    }));
            |]
            (fun (_ : _) -> false)
            ...
            [|
              (fun (_ : _) ->
                handle_default [||]
                  ...
                  (fun (_ : _) ->
                    {
                      d_identifiant = integer_of_string "4";
                      d_remuneration_mensuelle = money_of_cents_string "110000";
                      d_date_de_naissance = date_of_numbers 2001 1 1;
                      d_prise_en_charge = EffectiveEtPermanente ();
                      d_a_deja_ouvert_droit_aux_allocations_familiales = true;
                    }));
            |]
            ...
            [|
              (fun (_ : _) ->
                handle_default [||]
                  ...
                  (fun (_ : _) ->
                    {
                      d_identifiant = integer_of_string "5";
                      d_remuneration_mensuelle = money_of_cents_string "0";
                      d_date_de_naissance = date_of_numbers 2000 1 1;
                      d_prise_en_charge = EffectiveEtPermanente ();
                      d_a_deja_ouvert_droit_aux_allocations_familiales = true;
                    }));
            |]
  ...
  in
  {
    enfant1_out = enfant1_;
    enfant2_out = enfant2_;
    enfant3_out = enfant3_;
    enfant4_out = enfant4_;
    enfant5_out = enfant5_;
  }
...
let allocations_familiales (allocations_familiales_in : allocations_familiales_in) =
  ...
  let montant_verse_forfaitaire_par_enfant_ : money =
    try
      handle_default
        [| (fun (_ : _) -> montant_verse_forfaitaire_par_enfant_ ()) |]
        (fun (_ : _) -> true)
        (fun (_ : _) ->
          handle_default
            [|
  ...
  let montant_initial_metropole_majoration_ : enfant -> money =
    try
      handle_default
        [| (fun (_ : _) -> montant_initial_metropole_majoration_ ()) |]
        (fun (_ : _) -> true)
        (fun (_ : _) (param_ : enfant) ->
          try
            handle_default
                    ...
                    [|
                      (fun (_ : _) ->
                        handle_default [||]
                          ...
                          (fun (_ : _) ->
                            prestations_familiales_dot_base_mensuelle_ *$ decimal_of_string "0.0567"));
                      (fun (_ : _) ->
                        handle_default [||]
                          (fun (_ : _) ->
                            droit_ouvert_majoration_ param_
                            ...
                            && param_.age <! integer_of_string "16")
                          (fun (_ : _) ->
                            prestations_familiales_dot_base_mensuelle_ *$ decimal_of_string "0.0369"));
                    |]
                    (fun (_ : _) -> true)
...
let test9 (test9_in : test9_in) =
  let f_dot_date_courante_ : unit -> date =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> date_of_numbers 2020 5 1));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  let f_dot_enfants_ : unit -> enfant_entree array =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||]
            ...
            (fun (_ : _) ->
              [|
                {
                  d_identifiant = integer_of_string "0";
                  d_remuneration_mensuelle = money_of_cents_string "125800";
                  d_date_de_naissance = date_of_numbers 2001 7 27;
                  d_prise_en_charge = ServicesSociauxAllocationVerseeAuxServicesSociaux ();
                  d_a_deja_ouvert_droit_aux_allocations_familiales = true;
                };
                {
                  d_identifiant = integer_of_string "1";
                  d_remuneration_mensuelle = money_of_cents_string "176600";
                  d_date_de_naissance = date_of_numbers 2008 4 27;
                  d_prise_en_charge = ServicesSociauxAllocationVerseeAuxServicesSociaux ();
                  d_a_deja_ouvert_droit_aux_allocations_familiales = true;
                };
              |]));
      ...
      (fun (_ : _) -> raise EmptyError)
  in
  let f_dot_ressources_menage_ : unit -> money =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> money_of_cents_string "7578600"));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  let f_dot_residence_ : unit -> collectivite =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) -> handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> Guadeloupe ()));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  let f_dot_personne_charge_effective_permanente_est_parent_ : unit -> bool =
   fun (_ : unit) ->
    handle_default
      [| (fun (_ : _) -> handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> true)) |]
      (fun (_ : _) -> true)
      (fun (_ : _) -> false)
  in
  let result_ : interface_allocations_familiales_out =
    interface_allocations_familiales
      {
        date_courante_in = f_dot_date_courante_;
        enfants_in = f_dot_enfants_;
        enfants_a_charge_in = (fun (_ : unit) -> raise EmptyError);
        ressources_menage_in = f_dot_ressources_menage_;
        residence_in = f_dot_residence_;
        montant_verse_in = (fun (_ : unit) -> raise EmptyError);
        personne_charge_effective_permanente_est_parent_in =
          f_dot_personne_charge_effective_permanente_est_parent_;
        personne_charge_effective_permanente_remplit_titre_I_in =
          (fun (_ : unit) -> raise EmptyError);
      }
  in
  let f_dot_date_courante_ : date = result_.date_courante_out in
  let f_dot_enfants_ : enfant_entree array = result_.enfants_out in
  let f_dot_enfants_a_charge_ : enfant array = result_.enfants_a_charge_out in
  let f_dot_ressources_menage_ : money = result_.ressources_menage_out in
  let f_dot_residence_ : collectivite = result_.residence_out in
  let f_dot_montant_verse_ : money = result_.montant_verse_out in
  let f_dot_personne_charge_effective_permanente_est_parent_ : bool =
    result_.personne_charge_effective_permanente_est_parent_out
  in
  let f_dot_personne_charge_effective_permanente_remplit_titre__i_ : bool =
    result_.personne_charge_effective_permanente_remplit_titre_I_out
  in
  let (_ : unit) =
    if f_dot_montant_verse_ = money_of_cents_string "0" then () else raise AssertionFailed
  in
  ()

let test8 (test8_in : test8_in) =
  let f_dot_date_courante_ : unit -> date =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> date_of_numbers 2021 1 1));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
              ...
              [|
                {
                  d_identifiant = integer_of_string "0";
                  d_remuneration_mensuelle = money_of_cents_string "0";
                  d_date_de_naissance = date_of_numbers 2004 1 1;
                  d_prise_en_charge = EffectiveEtPermanente ();
                  d_a_deja_ouvert_droit_aux_allocations_familiales = true;
                };
                {
                  d_identifiant = integer_of_string "1";
                  d_remuneration_mensuelle = money_of_cents_string "0";
                  d_date_de_naissance = date_of_numbers 2005 1 1;
                  d_prise_en_charge = EffectiveEtPermanente ();
                  d_a_deja_ouvert_droit_aux_allocations_familiales = true;
                };
              |]));
  ...
  let f_dot_ressources_menage_ : unit -> money =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> money_of_cents_string "6994500"));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  let f_dot_residence_ : unit -> collectivite =
   fun (_ : unit) ->
    handle_default
      [| (fun (_ : _) -> handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> Metropole ())) |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  let f_dot_personne_charge_effective_permanente_est_parent_ : unit -> bool =
  ...
  in
  let (_ : unit) =
    if f_dot_montant_verse_ = money_of_cents_string "19774" then () else raise AssertionFailed
  in
  ()

let test7 (test7_in : test7_in) =
  let result_ : donnees_out =
    donnees
      {
        enfant1_in = (fun (_ : unit) -> raise EmptyError);
        enfant2_in = (fun (_ : unit) -> raise EmptyError);
        enfant3_in = (fun (_ : unit) -> raise EmptyError);
        enfant4_in = (fun (_ : unit) -> raise EmptyError);
        enfant5_in = (fun (_ : unit) -> raise EmptyError);
      }
  in
  let donnees_dot_enfant1_ : enfant_entree = result_.enfant1_out in
  let donnees_dot_enfant2_ : enfant_entree = result_.enfant2_out in
  let donnees_dot_enfant3_ : enfant_entree = result_.enfant3_out in
  let donnees_dot_enfant4_ : enfant_entree = result_.enfant4_out in
  let donnees_dot_enfant5_ : enfant_entree = result_.enfant5_out in
  let f_dot_date_courante_ : unit -> date =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> date_of_numbers 2021 2 1));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
      ...
      [|
        (fun (_ : _) ->
          handle_default [||]
            ...
            (fun (_ : _) ->
              [|
                donnees_dot_enfant1_;
                donnees_dot_enfant2_;
                donnees_dot_enfant3_;
                donnees_dot_enfant4_;
              |]));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  ...
  let f_dot_ressources_menage_ : unit -> money =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> money_of_cents_string "3000000"));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  ...
  in
  let (_ : unit) =
    if f_dot_montant_verse_ = money_of_cents_string "41751" then () else raise AssertionFailed
  in
  ()

let test6 (test6_in : test6_in) =
  let f_dot_date_courante_ : unit -> date =
   fun (_ : unit) ->
    handle_default
      [|
          ...
          handle_default [||]
            ...
            (fun (_ : _) ->
              [|
                {
                  d_identifiant = integer_of_string "0";
                  d_remuneration_mensuelle = money_of_cents_string "43900";
                  d_date_de_naissance = date_of_numbers 2009 11 10;
                  d_prise_en_charge = EffectiveEtPermanente ();
                  d_a_deja_ouvert_droit_aux_allocations_familiales = true;
                };
                {
                  d_identifiant = integer_of_string "1";
                  d_remuneration_mensuelle = money_of_cents_string "194900";
                  d_date_de_naissance = date_of_numbers 2020 9 25;
                  d_prise_en_charge = EffectiveEtPermanente ();
                  d_a_deja_ouvert_droit_aux_allocations_familiales = true;
                };
              |]));
  ...
  let f_dot_ressources_menage_ : unit -> money =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> money_of_cents_string "7883000"));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  ...
  in
  let (_ : unit) =
    if f_dot_montant_verse_ = money_of_cents_string "6625" then () else raise AssertionFailed
  in
  ()

let test5 (test5_in : test5_in) =
  let result_ : donnees_out =
    donnees
      {
        enfant1_in = (fun (_ : unit) -> raise EmptyError);
  ...
  let f_dot_enfants_ : unit -> enfant_entree array =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> [| donnees_dot_enfant1_ |]));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
...
let test4 (test4_in : test4_in) =
    ...
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||]
            (fun (_ : _) -> true)
            (fun (_ : _) -> [| donnees_dot_enfant1_; donnees_dot_enfant3_ |]));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  let f_dot_ressources_menage_ : unit -> money =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> money_of_cents_string "6725000"));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  ...
  in
  let (_ : unit) =
    if f_dot_montant_verse_ = money_of_cents_string "9937" then () else raise AssertionFailed
  in
  ()

let test3 (test3_in : test3_in) =
  let result_ : donnees_out =
    donnees
      {
        enfant1_in = (fun (_ : unit) -> raise EmptyError);
  ...
  let f_dot_ressources_menage_ : unit -> money =
   fun (_ : unit) ->
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> money_of_cents_string "6354000"));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  let f_dot_residence_ : unit -> collectivite =
   fun (_ : unit) ->
    handle_default
      [| (fun (_ : _) -> handle_default [||] (fun (_ : _) -> true) (fun (_ : _) -> Guyane ())) |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  let f_dot_personne_charge_effective_permanente_est_parent_ : unit -> bool =
  ...
  in
  let (_ : unit) =
    if f_dot_montant_verse_ = money_of_cents_string "2435" then () else raise AssertionFailed
  in
  ()

let test2 (test2_in : test2_in) =
  let result_ : donnees_out =
    donnees
      {
        enfant1_in = (fun (_ : unit) -> raise EmptyError);
    ...
    handle_default
      [|
        (fun (_ : _) ->
          handle_default [||]
            (fun (_ : _) -> true)
            (fun (_ : _) -> [| donnees_dot_enfant1_; donnees_dot_enfant2_; donnees_dot_enfant5_ |]));
      |]
      (fun (_ : _) -> false)
      (fun (_ : _) -> raise EmptyError)
  in
  ...
  in
  let (_ : unit) =
    if f_dot_montant_verse_ = money_of_cents_string "21627" then () else raise AssertionFailed
  in
  ()

let test1 (test1_in : test1_in) =
  let result_ : donnees_out =
    donnees
      {
        enfant1_in = (fun (_ : unit) -> raise EmptyError);
  ...
  in
  let (_ : unit) =
    if f_dot_montant_verse_ = money_of_cents_string "35126" then () else raise AssertionFailed
  in
  ()
