...
let get_def_key (def : Ast.definition)
  ...
  | _ -> 
    Errors.raise_spanned_error "Structs are not handled yet" (Pos.get_position def.definition_expr) 
    ...
    let rule_name =
      ...
      | None ->
          Desugared.Ast.RuleName.fresh
            ...
            | Some label -> label )
    in
    let is_exception (def : Ast.definition) = match def.Ast.definition_exception_to with
      | None -> false | _ -> true
    in
    (* If we had previously defined a rulename for this default definition during
       the elaboration of default exceptions, this trumps the newly generated name. *)
    let rule_name =
      if is_exception def then rule_name else
...
let gather_unnamed_exceptions (scope : Scopelang.Ast.ScopeName.t)
    (ctxt : Name_resolution.context)
    (defset : Desugared.Ast.ScopeDefSet.t) 
    (item : Ast.scope_use_item Pos.marked)
...
let generate_default_rulenames (scope : Scopelang.Ast.ScopeName.t)
  (ctxt : Name_resolution.context) 
  (defset : Desugared.Ast.ScopeDefSet.t)
  (rulemap : Desugared.Ast.RuleName.t Desugared.Ast.ScopeDefMap.t) 
  (item : Ast.scope_use_item Pos.marked)
  : Desugared.Ast.RuleName.t Desugared.Ast.ScopeDefMap.t =
  let is_exception (def : Ast.definition) = match def.Ast.definition_exception_to with
    | None -> false | _ -> true
  in
  match Pos.unmark item with
    ...
    (* This is a non-exception definition corresponding to an unnamed exception *)
      ...
      | Some label ->
        Errors.raise_spanned_error
          ...
          (Pos.get_position label)
      | None -> match Desugared.Ast.ScopeDefMap.find_opt scope_def rulemap with
        | Some _ ->
        ...
        | None -> 
          let fresh_name = Desugared.Ast.RuleName.fresh (Pos.same_pos_as "dummy" def.Ast.definition_name) in
...
let process_scope_use (ctxt : Name_resolution.context) (prgm : Desugared.Ast.program)
  ...
  let defset = 
    List.fold_left (gather_unnamed_exceptions scope_uid ctxt) 
      Desugared.Ast.ScopeDefSet.empty 
      use.scope_use_items
  in
  let scope_ctxt = Scopelang.Ast.ScopeMap.find scope_uid ctxt.scopes in
  let new_scope_ctxt = {scope_ctxt with default_rulemap =
    List.fold_left (generate_default_rulenames scope_uid ctxt defset)
