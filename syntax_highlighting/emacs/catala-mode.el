...
(define-generic-mode 'catala-mode-fr
  ...
  '(("\\<\\(selon\\|sous\s+forme\\|mais\s+en\s+remplaçant\\|fixé\\|par\\|inférieur\\|supérieur\\|varie\\|avec\\|on\s+a\\|soit\\|dans\\|tel\s+que\\|existe\\|pour\\|tout\\|de\\|si\\|alors\\|sinon\\|initial\\|parmi\\|est\s+maximum\\|est\s+minimum\\|combine\\|transforme\s+chaque\\|en\\|initialement\\|trie\\|par\s+ordre\s+\\(dé\\)?croissant\\|puis\\|impossible\\)\\>" . font-lock-builtin-face)
    ...
    ("\\<\\(entier\\|booléen\\|date\\|durée\\|argent\\|position_source\\|décimal\\|décret\\|loi\\|nombre\\|somme\\)\\>" . font-lock-type-face)
...
(define-generic-mode 'catala-mode-en
  ...
  '(("\\<\\(match\\|with\s+pattern\\|but\s+replace\\|fixed\\|by\\|down\\|up\\|varies\\|with\\|we\s+have\\|let\\|in\\|such\s+that\\|exists\\|for\\|all\\|of\\|if\\|then\\|else\\|initial\\|among\\|is\s+maximum\\|is\s+minimum\\|combine\\|map\s+each\\|to\\|initially\\|sort\\|in\s+\\(in\\|de\\)creasing\s+order\\|and\s+then\\|impossible\\)\\>" . font-lock-builtin-face)
    ("|[0-9]\\+-[0-9]\\+-[0-9]\\+|" . font-lock-constant-face)
    ("\\<\\(true\\|false\\)\\>" . font-lock-constant-face)
    ("\\<\\([0-9][0-9,]*\\(\\.[0-9]*\\|\\)\\)\\>" . font-lock-constant-face)
    ("\\(->\\|+.\\|+@\\|+^\\|+$\\|+\\|-.\\|-@\\|-^\\|-$\\|-\\|*.\\|*@\\|*^\\|*$\\|*\\|/.\\|/@\\|/$\\|/\\|!\\|>.\\|>=.\\|<=.\\|<.\\|>@\\|>=@\\|<=@\\|<@\\|>$\\|>=$\\|<=$\\|<$\\|>^\\|>=^\\|<=^\\|<^\\|>\\|>=\\|<=\\|<\\|=\\)" . font-lock-keyword-face)
    ("\\<\\(not\\|or\\|xor\\|and\\|\\$\\|%\\|year\\|month\\|day\\)\\>" . font-lock-keyword-face)
    ("\\<\\(integer\\|boolean\\|date\\|duration\\|money\\|code_location\\|decimal\\|number\\|sum\\)\\>" . font-lock-type-face)
    ("\\<[a-zéèàâùîôêœç][a-zéèàâùîôêœçA-ZÉÈÀÂÙÎÔÊŒÇ0-9_']*\\>" . font-lock-variable-name-face)
    ("\\<[A-ZÉÈÀÂÙÎÔÊŒÇ][a-zéèàâùîôêœçA-ZÉÈÀÂÙÎÔÊŒÇ0-9_']*\\>" . font-lock-function-name-face))
  '("\\.catala_en_raw$")
  '(catala-set-syntax-table)
