...
class CustomLexer(RegexLexer):
    ...
    tokens = {
        ...
        'code' : [
            ...
            (u'\\b(contains|number|sum|such\\s+that|exists|for|all|of|if|then|else|is|list\\s+empty|among|maximum|minimum|round|combine|map\\s+each|to|initially|sort|in\\s+(in|de)creasing\\s+order|and\\s+then|impossible)\\b', bygroups(Keyword.Declaration)),
