...
syn match Keyword contained "\<\(scope\|depends\s\+on\|declaration\|includes\|list\s\+of\|content\|type\|optional\|structure\|enumeration\|context\|rule\|under\s\+condition\|condition\|data\|consequence\|fulfilled\|equals\|assertion\|definition\|state\|label\|exception\|anything\|list\s+empty\)\>"
syn match Statement contained "\<\(match\|with\s\+pattern\|but\s\+replace\|fixed\|by\|decreasing\|increasing\|varies\|with\|we\s\+have\|let\|in\|such\s\+that\|exists\|for\|all\|of\|if\|then\|else\|initial\|among\|is\s+maximum\|is\s+minimum\|combine\|map\s+each\|to\|initially\)\>"
