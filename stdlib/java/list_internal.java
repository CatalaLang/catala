...
public class List_internal {
    ...
    public static class Globals {
        ...
        public static final CatalaFunction<CatalaArray<? extends CatalaValue<?>>, CatalaArray<CatalaValue<?>>> reverse
                = lst -> {
                    Object[] arro = lst.asArray();
                    CatalaValue<?>[] arr = Arrays.copyOf(arro, arro.length, CatalaValue[].class);
                    int len = arr.length;
                    if (len == 0) {
                        return CatalaValue.<CatalaArray<CatalaValue<?>>>cast(lst);
                    }
                    CatalaValue<?>[] ret = java.util.Arrays.copyOf(arr, len);
                    for (int i = 0; i < len; i++) {
                        ret[i] = arr[len - 1 - i];
                    }
                    return new CatalaArray<>(ret);
                };
    }
}
