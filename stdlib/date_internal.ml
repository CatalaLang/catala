...
(* Toplevel def of_ymd *)
let of_ymd : integer -> integer -> integer -> date =
 fun (y : integer) (m : integer) (d : integer) ->
  Dates_calc.Dates.make_date ~year:(Z.to_int y) ~month:(Z.to_int m)
    ~day:(Z.to_int d)
(* TODO: position argument + handle failure *)
...
let last_day_of_month : date -> date =
 fun (d : date) -> Dates_calc.Dates.last_day_of_month d
